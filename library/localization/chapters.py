...
def _chapters_from_sidecar(audio_path: Path) -> list[Chapter]:
    ...
    for i, ch in enumerate(raw_chapters):
        ...
        chapters.append(
            Chapter(index=i, title=title, start_ms=start_ms, end_ms=start_ms + length_ms)
        )
    return chapters


def split_chapter(audio_path: Path, chapter: Chapter, output_dir: Path | None = None) -> Path:
