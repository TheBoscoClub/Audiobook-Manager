...
def _parse_ratio_progress(line, pattern, base, span, tracker, op_id, state, fmt):
    ...
    if total > 0:
        ...
        _update_if_advanced(tracker, op_id, state, progress, fmt.format(current=current, total=total))
    return True


def _parse_count_progress(line, pattern, base, divisor, cap, tracker, op_id, state, fmt):
...
def _rebuild_queue_work(tracker, operation_id):
    ...
    def on_line(line):
        ...
        scanned = _parse_count_progress(
            line, _SCANNING_RE, 5, 50, 80, tracker, operation_id, state,
    ...
    handle_result(
        tracker, operation_id, result,
        {"queue_size": counters["queue_size"], "output": result["output"]},
        "Queue rebuild failed",
    )


@utilities_ops_maintenance_bp.route("/api/utilities/rebuild-queue-async", methods=["POST"])
@admin_if_enabled
...
def _cleanup_indexes_work(tracker, operation_id, *, dry_run):
    ...
    def on_line(line):
        ...
        if _parse_ratio_progress(
            line, _IDX_PROGRESS_RE, 5, 85, tracker, operation_id, state,
            "Checking entries: {current}/{total}",
        ):
            return
        checked = _parse_count_progress(
            line, _CHECKING_RE, 5, 100, 85, tracker, operation_id, state,
    ...
    result = run_with_progress(
        cmd, line_callback=on_line, timeout_secs=600,
        operation_name="Cleanup", env={**os.environ, "TERM": "dumb"},
    )
    handle_result(
        tracker, operation_id, result,
        {"entries_removed": counters["removed"], "dry_run": dry_run, "output": result["output"]},
        "Cleanup failed",
    )


@utilities_ops_maintenance_bp.route("/api/utilities/cleanup-indexes-async", methods=["POST"])
@admin_if_enabled
...
def _sort_fields_work(tracker, operation_id, *, dry_run):
    ...
    def on_line(line):
        ...
        if loaded is not None:
            tracker.update_progress(operation_id, 10, f"Found {loaded} audiobooks to process")
            return
        if _parse_ratio_progress(
            line, _SORT_PROGRESS_RE, 10, 80, tracker, operation_id, state,
            "Analyzing: {current}/{total}",
        ):
            return
        processed = _parse_count_progress(
            line, _PROCESSING_RE, 10, 20, 85, tracker, operation_id, state,
    ...
    result = run_with_progress(
        cmd, line_callback=on_line, timeout_secs=300,
        operation_name="Sort field population",
    )
    handle_result(
        tracker, operation_id, result,
        {"fields_updated": counters["updated"], "dry_run": dry_run, "output": result["output"]},
        "Sort field population failed",
    )


@utilities_ops_maintenance_bp.route("/api/utilities/populate-sort-fields-async", methods=["POST"])
@admin_if_enabled
...
def _export_audible_library(library_export):
    ...
    if export_result.returncode != 0:
        ...
        return False, f"Failed to export Audible library (code {export_result.returncode}): {error_msg}"
...
def _asin_match_on_line(tracker, operation_id, state, counters, line):
    ...
    if not line:
        return
    if _parse_ratio_progress(
        line, _ASIN_PROGRESS_RE, 30, 60, tracker, operation_id, state,
        "Matching: {current}/{total} audiobooks",
    ):
        return
    count = _parse_count_progress(
        line, _ASIN_PROCESSING_RE, 30, 10, 85, tracker, operation_id, state,
...
def _run_asin_matching(tracker, operation_id, library_export, dry_run):
    ...
    library_script = _project_root / "backend" / "migrations" / "populate_asins_from_library.py"
    ...
    cmd = [
        sys.executable, "-u", str(library_script),
        "--library", str(library_export),
        "--db", str(AUDIOBOOKS_DATABASE),
        "--threshold", "0.6",
    ]
    if dry_run:
        cmd.append("--dry-run")
...
def _populate_asins_work(tracker, operation_id, *, dry_run):
    ...
    try:
        ...
        tracker.update_progress(operation_id, 30, "Library exported, starting match process...")
...
@utilities_ops_maintenance_bp.route("/api/utilities/populate-asins-async", methods=["POST"])
@admin_if_enabled
...
def _find_duplicates_work(tracker, operation_id, *, dry_run):
    ...
    def on_line(line):
        ...
        if not line:
            return
        if _parse_ratio_progress(
            line, _DUP_PROGRESS_RE, 5, 85, tracker, operation_id, state,
            "Comparing: {current}/{total} files",
        ):
            return
        scanned = _parse_count_progress(
            line, _DUP_SCANNING_RE, 5, 50, 80, tracker, operation_id, state,
            "Scanned {count} files",
        )
        if scanned is not None:
        ...
        if found is not None:
            tracker.update_progress(operation_id, 20, f"Found {found} source files to analyze")
            return
    ...
    result = run_with_progress(
        cmd, line_callback=on_line, timeout_secs=600,
        operation_name="Duplicate scan", env={**os.environ, "TERM": "dumb"},
    )
    handle_result(
        tracker, operation_id, result,
        {"duplicates_found": counters["duplicates"], "dry_run": dry_run, "output": result["output"]},
        "Duplicate scan failed",
    )


@utilities_ops_maintenance_bp.route("/api/utilities/find-source-duplicates-async", methods=["POST"])
@admin_if_enabled
