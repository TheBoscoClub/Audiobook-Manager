...
def _cleanup_indexes_work(tracker, operation_id, *, dry_run):
    ...
    def on_line(line):
        ...
        checked = _parse_count_progress(
            line, _CHECKING_RE, 5, 100, 85, tracker, operation_id, state, "Verified {count} entries"
        )
...
def _sort_fields_work(tracker, operation_id, *, dry_run):
    ...
    result = run_with_progress(
        cmd, line_callback=on_line, timeout_secs=300, operation_name="Sort field population"
    )
    handle_result(
        tracker,
        operation_id,
...
def _run_asin_matching(tracker, operation_id, library_export, dry_run):
    ...
    result = run_with_progress(
        cmd, line_callback=on_line, timeout_secs=300, operation_name="ASIN matching"
    )
    handle_result(
        tracker,
        operation_id,
...
def _find_duplicates_work(tracker, operation_id, *, dry_run):
    ...
    def on_line(line):
        ...
        if _parse_ratio_progress(
            ...
            return
        scanned = _parse_count_progress(
            line, _DUP_SCANNING_RE, 5, 50, 80, tracker, operation_id, state, "Scanned {count} files"
        )
        if scanned is not None:
