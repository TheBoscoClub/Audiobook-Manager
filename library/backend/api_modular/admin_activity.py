...
def _get_book_titles(audiobook_ids: set) -> dict[str, str | None]:
    ...
    try:
        ...
        cursor = conn.execute(  # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
            f"SELECT id, title FROM audiobooks WHERE id IN ({placeholders})",  # nosec B608 — placeholders = ",".join("?"*N) of only `?` chars; int_ids is a list of ints; no user-controlled string interpolation  # noqa: S608
            int_ids,
        )
