...
def _get_affected_book_ids(conn: sqlite3.Connection, entity_type: str, entity_id: int) -> list[int]:
    ...
    rows = conn.execute(  # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
        f"SELECT book_id FROM {table} WHERE {col} = ?",  # nosec B608 — table/col from code-defined {author|narrator} branch above; entity_id is int parameter-bound  # noqa: S608
        (entity_id,),
    ).fetchall()
...
def _merge_entities(
    ...
    for sid in source_ids:
        ...
        conn.execute(  # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
            f"DELETE FROM {entity_table} WHERE id = ?",  # nosec B608 — entity_table is "authors" or "narrators" literal from code-defined caller at L356/L516; sid is int parameter-bound  # noqa: S608
            (sid,),
        )
...
def _verify_entities_exist(conn, entity_table, target_id, source_ids, label):
    ...
    target = conn.execute(  # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
        f"SELECT * FROM {entity_table} WHERE id = ?",  # nosec B608 — entity_table is "authors" or "narrators" literal from callers at L356/L516; target_id is parameter-bound  # noqa: S608
        (target_id,),
    ).fetchone()
    if not target:
    ...
    for sid in source_ids:
        src = conn.execute(  # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
            f"SELECT id FROM {entity_table} WHERE id = ?",  # nosec B608 — same allowlist as above; sid is parameter-bound  # noqa: S608
            (sid,),
        ).fetchone()
        if not src:
