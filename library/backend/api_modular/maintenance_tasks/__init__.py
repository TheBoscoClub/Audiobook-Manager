...
from .base import ExecutionResult, MaintenanceRegistry, MaintenanceTask, ValidationResult
...
for _importer, _modname, _ispkg in pkgutil.iter_modules([str(_pkg_dir)]):
    ...
    if _modname not in _ALLOWED_TASK_MODULES or not _MODNAME_RE.match(_modname):
        raise ValueError(f"Refusing to import unrecognized maintenance task module: {_modname!r}")
