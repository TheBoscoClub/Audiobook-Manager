...
def update_position(audiobook_id: int):
    ...
    # Defense-in-depth: reject near-zero positions (1-4999ms) that indicate
    # a frontend bug where auto-save fires at the start of playback.
    # Exactly 0 is allowed (intentional clear on book completion).
    if 0 < position_ms < 5000:
        return jsonify({"error": "Position too small (< 5s), likely erroneous"}), 422
