...
def main():
    ...
    server = http.server.HTTPServer(
        (
            "0.0.0.0",  # noqa: S104 — HTTPS server, all-interface bind intentional for multi-NIC deployments  # nosec B104 — bind 0.0.0.0 — intentional; service is fronted by Caddy/TLS reverse proxy, not exposed directly
            HTTPS_PORT,
        ),
        handler,
    )
