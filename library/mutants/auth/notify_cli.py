...
from auth import Notification, NotificationRepository, NotificationType
...
Examples:
    ...
    create_parser.add_argument(
        "--user", "-u", type=int, help="Target user ID (for personal notifications)"
    )
    create_parser.add_argument(
        "--expires", "-e", help="Expiry datetime (ISO format: YYYY-MM-DDTHH:MM:SS)"
    )
    create_parser.add_argument(
        "--no-dismiss", action="store_true", help="Make notification non-dismissable"
    )
    create_parser.add_argument(
        "--priority", "-p", type=int, default=0, help="Priority (higher = shown first)"
    )
