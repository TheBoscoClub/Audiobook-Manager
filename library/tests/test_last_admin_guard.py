...
class TestLastAdminGuard:
    def test_count_admins_returns_correct_count(self, auth_db):
        ...
        User(
            username="admin1_lag", auth_type=AuthType.TOTP, auth_credential=b"s", is_admin=True
        ).save(auth_db)
        User(
            username="admin2_lag", auth_type=AuthType.TOTP, auth_credential=b"s", is_admin=True
        ).save(auth_db)
        User(
            username="regular_lag", auth_type=AuthType.TOTP, auth_credential=b"s", is_admin=False
        ).save(auth_db)
