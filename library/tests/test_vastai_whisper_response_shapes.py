...
def test_faster_whisper_top_level_words_shape(audio_file: Path):
    ...
    with patch(
        "library.localization.stt.vastai_whisper.requests.post", return_value=_mock_post(payload)
    ):
...
def test_whisper_cpp_nested_segments_words_shape(audio_file: Path):
    payload = {
        ...
        "duration": 2.0,
        "segments": [
            {"id": 0, "text": "你好", "words": [{"word": "你好", "start": 0.0, "end": 0.8}]},
            {"id": 1, "text": "世界", "words": [{"word": "世界", "start": 1.0, "end": 1.7}]},
        ],
    }
    with patch(
        "library.localization.stt.vastai_whisper.requests.post", return_value=_mock_post(payload)
...
def test_string_typed_timestamps_dont_crash(audio_file: Path):
    ...
    payload = {"duration": "1.0", "words": [{"word": "test", "start": "0.1", "end": "0.4"}]}
    with patch(
        "library.localization.stt.vastai_whisper.requests.post", return_value=_mock_post(payload)
    ):
        provider = VastaiWhisperSTT(host="127.0.0.1")
...
def test_text_field_alias_used_when_word_missing(audio_file: Path):
    ...
    payload = {
        "duration": 0.5,
        "segments": [{"words": [{"text": "alt", "start": 0.0, "end": 0.4}]}],
    }
    with patch(
        "library.localization.stt.vastai_whisper.requests.post", return_value=_mock_post(payload)
    ):
...
def test_duration_falls_back_to_last_word_end(audio_file: Path):
    payload = {
        "words": [{"word": "a", "start": 0.0, "end": 0.5}, {"word": "b", "start": 0.6, "end": 1.4}]
    }
    with patch(
        "library.localization.stt.vastai_whisper.requests.post", return_value=_mock_post(payload)
    ):
