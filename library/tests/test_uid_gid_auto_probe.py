...
def test_install_sh_defaults_matched_pair():
    ...
    body = _read("install.sh")
    uid_match = re.search(
        r'AUDIOBOOKS_PREFERRED_UID="\$\{AUDIOBOOKS_PREFERRED_UID:-(\d+)\}"', body
    )
    gid_match = re.search(
        r'AUDIOBOOKS_PREFERRED_GID="\$\{AUDIOBOOKS_PREFERRED_GID:-(\d+)\}"', body
    )
...
def test_dockerfile_defaults_match():
    ...
    assert uid_match.group(1) == gid_match.group(1), (
        f"Dockerfile UID/GID defaults do not match: "
        f"{uid_match.group(1)} vs {gid_match.group(1)}"
    )
