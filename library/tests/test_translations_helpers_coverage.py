...
class TestTranslateUniqueSeries:
    def test_no_series_books_returns_empty(self):
        ...
        mapping, unique = tr._translate_unique_series(
            translator, [{"id": 1, "series": None}], "es"
        )
        assert mapping == {}
        translator.translate.assert_not_called()
...
class TestApplyTranslations:
    ...
    def test_empty_series_maps_to_empty_string(self, seeded_db: Path):
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT series_display FROM audiobook_translations "
...
class TestTranslateBatchFieldWithMap:
    ...
    def test_maps_translations_back_to_original_positions(self):
        ...
        result = tr._translate_batch_field_with_map(
            translator, ["one", "", "two"], "fr"
        )
...
class TestFindExistingTranslations:
    ...
    def test_intersects_translated_with_book_ids(self, seeded_db: Path):
        ...
        try:
            ...
            conn.commit()
            existing = tr._find_existing_translations(
                conn, "zh-Hans", [{"id": 1}, {"id": 2}]
            )
            assert existing == {1}
        finally:
            conn.close()
...
class TestBatchNothingToDo:
    def test_shape(self, flask_app):
        with flask_app.app_context():
            resp = tr._batch_nothing_to_do_response(
                books=[{"id": 1}, {"id": 2}],
                existing={1},
            )
            data = resp.get_json()
