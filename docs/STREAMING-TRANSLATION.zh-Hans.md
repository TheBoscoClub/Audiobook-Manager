...
│  └─────────────────────────────────────────────────────────┘      │
│                                                                    │
│  运行环境：Vast.ai 和/或 RunPod（双供应商对等）—— 或自托管        │
└───────────────────────────────────────────────────────────────────┘
```
...
优先级（数值越小越紧急）：
  ...
  2  P2 —— 回填。产出上一段已翻译尾部与光标之间的分段。P0 满足
         后才运行，以保证侧边字幕面板和将来向后拖动时的
         上下文连贯。

跳转超出缓冲范围时：现有待处理分段降级为 P2；新光标前方 6 个分段
被提升或新插入为 P0；章末剩余部分排入 P1；上一段尾部与新光标之间
的空缺排入 P2。

停止播放时：所有待处理分段降级为 P2（回填策略保留已排入的工作
以便将来继续播放和补全侧边面板）。
```

工作节点的领取顺序 —— `ORDER BY priority, chapter, segment` —— 保持不变；
v8.3.2 变更的只是每个优先级的语义，从"章节角色"变为"与光标的关系"。
