#!/bin/bash
# Data migration 003: translation_queue heartbeat + chapter total (v8.2.2)
#
...
_dm003_has_column() {
    local col="$1"
    _dm003_sqlite "PRAGMA table_info(translation_queue);" 2>/dev/null \
...
if $_dm003_need_progress; then
    _dm003_sqlite "ALTER TABLE translation_queue ADD COLUMN last_progress_at TIMESTAMP;" \
        && _dm003_sqlite "UPDATE translation_queue SET last_progress_at = COALESCE(started_at, created_at);" \
        && _dm003_sqlite "CREATE INDEX IF NOT EXISTS idx_tq_last_progress ON translation_queue(last_progress_at);" \
        && echo "  Added translation_queue.last_progress_at"
fi

if $_dm003_need_total; then
    _dm003_sqlite "ALTER TABLE translation_queue ADD COLUMN total_chapters INTEGER;" \
