import tempfile, os, signal, time, logging, threading, subprocess, traceback
...
def _deadman_loop():
    while True:
        ...
        if idle > IDLE_SHUTDOWN_SEC:
            logging.warning('Dead-man TTL: idle %ds > %ds — terminating container',
                            int(idle), IDLE_SHUTDOWN_SEC)
...
if IDLE_SHUTDOWN_SEC > 0:
    ...
    log.info('Dead-man TTL enabled: %ds (terminates container via SIGTERM PID 1)', IDLE_SHUTDOWN_SEC)
else:
...
def transcribe():
    ...
    try:
        ...
        log.info('transcribed in %.1fs duration=%.1fs words=%d',
                 time.time() - t0, info.duration, len(words))
        return jsonify({
