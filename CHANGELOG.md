...
- **`docs/EMAIL-SETUP.md` sample SMTP key is now an obvious placeholder** — `re_DJR8Prne_SAMPLE_SEND_ONLY_KEY` replaced with `YOUR_RESEND_API_KEY_HERE`. Eliminates any chance of grep-for-prefix collisions mistaking the docs for a real key
...
- **Semgrep SSRF warning on RunPod warmth probe suppressed**: `streaming_translate.py::_probe_runpod_warmth` builds `https://api.runpod.ai/v2/{endpoint}/health` from a hardcoded scheme/host and an admin-only env var (`AUDIOBOOKS_RUNPOD_STREAMING_WHISPER_ENDPOINT`); no user input reaches the URL, and the call is authenticated with a Bearer API key. Added `# nosemgrep: python.lang.security.audit.dynamic-urllib-use-detected.dynamic-urllib-use-detected` with an inline rationale comment
