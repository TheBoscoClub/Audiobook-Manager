...
- **Mutation-testing scope extended to the guard modules** (`Audiobook-Manager-6y9`): `deepl_translate.py` and `common_utils/mail_identity.py` join `quota.py` — 778 mutants, 529 killed, 249 survived, 0 uncovered. The gate lives in `ci.yml`, which already runs on push, nightly at 05:00 UTC and on dispatch, so it gets both cadences by placement. At 2m6s for 322 mutants it stays well inside the 4m30s the Python Tests jobs take in parallel, so the wider scope costs no wall-clock on a push

- **Branch protection: required status checks removed, everything else kept**: `main` keeps required signed commits, no force-push, no deletion and conversation resolution. The 12 required status checks were dropped because they protected nothing here — the repository is public with exactly one collaborator holding push access, so an unauthorised user cannot push regardless, and with `enforce_admins: false` the checks only emitted `Bypassed rule violations` on every direct push. Secret scanning, push protection and Dependabot alerts remain enabled, and Actions default to read-only permissions

### Fixed

- **Tests derived from surviving mutants, not from re-reading the code** (`Audiobook-Manager-6y9`): mutation testing found live mutants inside guards that a full green suite had pronounced verified. `resolve_sender` 25 → 16 survivors, `_invalidate_glossary` 13 → 6, `_merge_translations_into_output` 14 → 7. The new tests pin what the old ones never looked at: that a refusal message names the variable, the mechanism and the fix (the operator's only signal when mail refuses); that `rsplit("@", 1)` is load-bearing, since `split` would read `a@b@localhost` as domain `b@localhost` and **accept** it; that glossary invalidation clears the in-memory id, the persisted id, and leaves `_glossary_resolved` True so the retry cannot re-attach a glossary; and that every identity result in a batch is skipped, not just the first
...
- **`ruff format` was rewriting source into Python 3.14-only syntax while CI tests 3.12 and 3.13**: `pyproject.toml` set `target-version = "py314"`, which licenses ruff to emit any syntax legal on 3.14 — including stripping the parentheses from `except (A, B):` per PEP 758. That form is a **SyntaxError on 3.12/3.13**, so a formatted file imports cleanly on the dev machine and fails to parse on two of the three matrix entries in `.github/workflows/ci.yml`. Caught in this cycle when formatting a new helper produced `except ValueError, TypeError:` and the existing `test_no_py2_except_comma` guard fired. `target-version` is now `py312` — the **oldest** interpreter CI runs — with a comment saying to raise it only when the matrix floor rises. The guard's own docstring was rewritten: its stated rationale (pre-3.14 `except A as B:` shadowing) no longer applies, and describing the real failure — a CI-only break invisible locally — is what makes the guard actionable

- **`deepl_quota.refresh_from_api()` had no caller — the quota row was never reconciled** (`Audiobook-Manager-2s6`): the routine existed, was tested, and was invoked from nowhere outside its own tests, so `last_api_check` had been NULL since the table was created in April and `chars_used` was a purely local tally that drifts from DeepL's own counter (which resets each billing period). `check_before_translate()` — the gate that blocks translation at 99% — was deciding on a number nobody had ever verified against the vendor. It now calls a new `_reconcile_if_stale()` first: at most hourly, only when credentials are configured, throttled to one attempt per five minutes so a failing usage endpoint is not retried on every call, and failing open with a WARNING because a usage-endpoint outage must never block translation

- **`import_to_db._cleanup_orphaned_covers()` would delete an entire cover library when mispointed** (`Audiobook-Manager-d40`): the routine `unlink()`s every file in `COVER_DIR` the database does not reference. With a temp test database that is *every cover*, and the suite resolved `COVER_DIR` from the ambient environment — so running it from a directory where config fell back to production defaults attempted to delete the real library's cover art, stopped only by filesystem permissions that would not apply under the documented `sudo -u audiobooks pytest` invocation. Two guards now: the routine **refuses** a sweep that would remove every file present or that finds no referenced covers at all, printing what it declined to do; and `library/tests/conftest.py` pins `COVER_DIR`, `AUDIOBOOKS_COVERS`, `AUDIOBOOKS_LIBRARY` and `AUDIOBOOKS_DATA` into a per-run temp tree at module scope — a fixture would be too late, since `config.py` resolves them at import

- **`scripts/regenerate-doc-pdfs.sh` printed `ok` when pandoc failed** (`Audiobook-Manager-p1k`): pass/fail was derived solely from grepping stderr for `Missing character`, and `set -uo pipefail` without `-e` meant the assignment always succeeded, so pandoc missing, xelatex crashing, a missing `pandoc-header.tex`, a LaTeX error or OOM all reported success with no PDF on disk — a drift detector that failed silently on every failure except the one it grepped for. It now checks `PIPESTATUS[0]` and asserts the PDF exists and is non-empty. A pre-existing unguarded `cd` (SC2164) in the same script, the same bug class, was fixed alongside

### Security

- **Stale DeepL credential material removed from every host and VM location** (`Audiobook-Manager-a0s`, `Audiobook-Manager-8x4`): config backups on the dev and QA VMs (`/etc/audiobooks/*.bak*`) and on the host (`/root/audiobooks-conf-backups/`) carried an uncommented `AUDIOBOOKS_DEEPL_API_KEY` assignment holding a superseded key — neither the current Pro key nor the free-tier key expunged earlier. All were deleted. Verified by enumerating every DeepL assignment across host and both VMs and comparing each to the current Pro key by identity, with a positive control proving the search could still find a planted copy: host now has exactly one assignment (the Pro key) and the VMs have none
...
- **`docs/EMAIL-SETUP.md` documented three already-fixed send sites as a "Known defect"** (`Audiobook-Manager-t0a`, found by cloud review of PR #150): the TL;DR and a fenced warning box told operators to treat admin alerts and admin replies as "non-functional on the relay path", citing `auth_email.py:182`, `:213` and `inbox_cli.py:181` — line numbers that no longer point at `starttls()` calls at all, because v8.4.3 shipped the guard at `auth_email.py:190`, `:225` and `inbox_cli.py:188`. The box is now past-tense and version-stamped, and a new **Sender identity is required** section documents the `SMTP_FROM` contract. `etc/audiobooks.conf.example` promotes `SMTP_FROM` from a commented-out suggestion to a required setting, and `docs/TROUBLESHOOTING.md` names the new refusal message so it is searchable
...
- **Dependency floors raised via Dependabot**: `cryptography>=50.0.0` and `filelock>=3.32.2`
  ...
  PRs #128, #129, #130, #131. The stale inline comment on the `cryptography` line (still
  describing 49.0.0 as current) was corrected in both files
...
- **Restored the deleted `[8.4.0.0]` changelog section**: commit `e9601144` (the 8.4.0.1
  ...
  changelog. Recovered verbatim from `git show v8.4.0.0:CHANGELOG.md`

- **Latent mypy failure in `test_player_navigation_persistence.py`**: the selenium
  import-fallback lines `By = None` and `WebDriverWait = None` carried
  `# type: ignore[assignment]`, but assigning `None` to a class also emits `misc` — invisible
...
- **CI security gates can now actually fail**: the `bandit` job in `python-security.yml` was
  ...
  `pip-audit` twice — neither could report failure regardless of findings. Worse, the tuned
  `.bandit` config (audited per-rule skips from 2026-04-17) was gitignored, so CI checkouts
  scanned with zero suppressions and the masked run had been erroring/failing invisibly the
  whole time. `.bandit` is now tracked (project contract per the "does CI read this file?"
  test in the gitignore policy), the bandit job runs an all-severities report artifact plus a
  gating `-ll` (MEDIUM+) pass, and `pip-audit` audits all three requirement files ungated.
  The 11 remaining LOW `B110` findings are tracked for per-site audit; `pyproject.toml`'s
  false claim that CI enforces `ruff format` was corrected (gate decision also tracked)
...
- **Phase 10a VM audit — `scripts/batch-translate.py::_configure_env` bash-quoted value injection**: `_configure_env` read `/etc/audiobooks/audiobooks.conf` (format: `KEY="value"`) and called `os.environ.setdefault(key.strip(), val.strip())`. The `val.strip()` preserved the surrounding double-quotes, injecting `'"5001"'` as `AUDIOBOOKS_API_PORT`; downstream `config.py::int(get_config("AUDIOBOOKS_API_PORT"))` raised `ValueError: invalid literal for int() with base 10: '"5001"'`. Fixed by stripping quotes after whitespace: `cleaned = val.strip().strip('"').strip("'")`. Detected via `pytest` on the test VM under `test-audiobook-cachyos` (pristine BTRFS restore + fresh `install.sh --system`) — contamination caused 77 failures across the full suite; production was unaffected since batch-translate is a standalone script run interactively, not via systemd
- **Phase 10a VM audit — repo-structure tests fail in deployed environments**: 15 test modules (e.g. `test_upgrade_behavioral.py`, `test_cachebust_automation.py`, `test_caddy_port_substitution.py`) open `upgrade.sh`, `install.sh`, `caddy/audiobooks.conf`, and `systemd/` from a project checkout. When `pytest` runs from `/opt/audiobooks/library/` after `install.sh`, none of those files exist and every test raises `FileNotFoundError`. Added `pytestmark = pytest.mark.requires_repo_source` to all 15 modules and an auto-skip gate in `conftest.py::pytest_collection_modifyitems` that skips the entire marker cohort when `PROJECT_ROOT / "upgrade.sh"` is absent — clean `SKIP` instead of `ERROR`
- **Phase 10a VM audit — `test_provider_agnostic.py` `FileNotFoundError` in deployed environment**: `test_install_sh_template_documents_runpod_stt_stubs` opened `REPO_ROOT / "install.sh"` which resolves to `/opt/audiobooks/install.sh` (not present). Fixed with `@pytest.mark.skipif(not (REPO_ROOT / "install.sh").exists(), reason="...")`. Also fixed `ModuleNotFoundError: No module named 'library'` for the 3 tests that used `from library.backend.api_modular import …` by adding `PROJECT_ROOT` to `sys.path` at the top of `conftest.py`
- **Phase 10a VM audit — `conftest.py` `sys.path` missing `PROJECT_ROOT`**: `pytest` run from `library/` had `LIBRARY_DIR` on `sys.path` but not the parent `PROJECT_ROOT`; imports of the form `from library.backend.api_modular …` failed with `ModuleNotFoundError`. Added `sys.path.insert(0, str(PROJECT_ROOT))` at conftest init
...
- **Production-code Pyright Optional narrowing across 14 modules**: full Phase 6 type-error sweep eliminated all 50 prod-code Pyright errors. Patterns fixed: (a) Optional ID/object narrowing after `if x is None: return` guards via `assert x is not None` in `library/auth/cli.py` (User.id), `library/backend/api_modular/auth_registration.py` (claim-token resolver result), `library/backend/api_modular/auth_webauthn.py` (verify_webauthn_credential result), `library/backend/api_modular/subtitles.py` (loader Union narrowing), `library/backend/api_modular/utilities_crud.py` (validator result). (b) `_auth_mod()` accessors in `auth_admin.py`, `auth_registration.py`, `auth_webauthn.py` now raise `RuntimeError` if the parent `api_modular.auth` module is genuinely absent — eliminates 48 chained `None.attribute` errors at email/repo/setup_totp call sites (raise is unreachable at runtime — auth_admin/registration/webauthn are only imported transitively from `api_modular.auth`). (c) Lazy-import-with-fallback patterns in `library/scanner/import_single.py`, `library/scanner/add_new_audiobooks.py`, `library/backend/api_modular/position_sync.py`, `library/scripts/google_play_processor.py`, and `library/localization/translation/deepl_translate.py` migrated to `TYPE_CHECKING`-guarded imports or explicit `Callable[..., Any] | None` return annotations — eliminates `reportPossiblyUnboundVariable` for `OpenLibraryClient`, `MP3`/`APIC`/`OggOpus`/`Picture`, `PositionRepository` family, and `GlossaryError`. (d) `library/auth/database.py` sqlcipher3 fallback migrated to `TYPE_CHECKING` guard so `sqlcipher.Connection`/`sqlcipher.connect`/`sqlcipher.DatabaseError` type expressions resolve. (e) `library/auth/totp.py:107` qrcode `img.save(buffer, format="PNG")` annotated `# type: ignore[call-arg]` — qrcode's stub omits the kwarg but runtime passes it through to PIL unchanged. (f) `library/backend/api_modular/__init__.py:193` `init_queue(database_path, project_root)` guarded with `if project_root is None: raise RuntimeError` because the parameter has a `None` default but the callee requires `Path`. (g) `library/backend/api_modular/auth_admin.py` audit-log endpoint list comprehension now filters `if e is not None` since `AuditLog.from_row` is typed `Optional[AuditLog]` (the None case is unreachable when iterating fetched rows, but the type checker can't see that)
- **Test-suite Pyright Optional narrowing across 32 test modules**: companion sweep to the production-code fix eliminated all remaining Pyright errors in the test suite. Eight commits narrowed Optional return values and added `# type: ignore` annotations with explanatory codes across `test_auth.py`, `test_auth_api.py`, `test_auth_api_coverage.py`, `test_auth_api_extended.py`, `test_auth_webauthn.py`, `test_auth_webauthn_flows.py`, `test_admin_user_management.py`, `test_enrich_single.py`, `test_multi_session.py`, `test_player_navigation_persistence.py`, `test_maintenance_scheduler_coverage.py`, `test_inbox.py`, `test_auth_ui_e2e.py`, `test_audit_log.py`, `test_auth_security.py`, `test_user_settings.py`, `test_progress_streaming.py`, `test_session_grace_period.py`, `test_credential_reset_claim_lifecycle.py`, `test_asin.py`, `test_auth_docker.py`, `test_auth_backup.py`, `test_auth_cleanup_task.py`, `test_auth_database.py`, `test_auth_integration.py`, `test_auth_method_switch_claim_lifecycle.py`, `test_auth_performance.py`, `test_add_new_audiobooks.py`, `test_batch_translate.py`, and others. Patterns identical to the production sweep (`assert x is not None`, explicit `if x is not None:` guards, `# type: ignore[union-attr]` at unavoidable call sites). Pyright now reports 0 errors and 0 warnings across the entire codebase — both production and test code
- **Pyright warning suppressions for 95 stub-gap and runtime-dynamic sites**: four targeted passes added `# type: ignore[attr-defined]` and `# type: ignore[import-untyped]` at sites where third-party stubs are absent or incomplete and the runtime behaviour is verified correct. Categories: (1) `sqlcipher3` / `qrcode` stub-gap warnings in `library/auth/database.py` (4 sites) and `library/auth/totp.py` (1 site) — Connection/connect/DatabaseError and the `constants` submodule exist at runtime but are not in the published stubs. (2) Optional-dependency import warnings for `gevent` / `torch` / `whisper` / `mutagen` — imports already guarded by `try/ImportError` or `HAS_*` checks; suppressed in `library/backend/api_server.py`, `library/backend/api_modular/websocket.py`, and the localization GPU modules. (3) Dynamic `sys.path` import warnings in modules and tests that add `library/` to `sys.path` at runtime — suppressed with `# type: ignore[import-not-found]` at the import sites where the module is reachable at runtime. (4) Dynamic-attribute warnings for Flask `Blueprint`, `Module`, and `Response` objects in `library/backend/api_modular/auth_registration.py`, `auth_admin.py`, and `auth_webauthn.py` — attributes attached at registration time are not visible to Pyright's static analysis but are always present at runtime
- **webauthn / passkey fixture `assert-before-if` regression in `test_auth_webauthn_flows.py`**: Phase 6b's Optional-narrowing pass inserted `assert existing is not None` immediately before `if existing: return existing` in the `_make_passkey_user` and `_make_fido2_user` test helpers. Because the intent of those helpers is to create the user on first call and return the existing one on subsequent calls, `existing` is legitimately `None` on the first call — making the assert a hard `AssertionError` that broke 7 tests (`TestLoginWebAuthnBegin::test_begin_passkey_success`, `test_begin_fido2_success`, `TestLoginWebAuthnComplete::test_complete_success`, `test_complete_remember_me_false`, `test_complete_fido2_user`, `test_complete_verification_fails`, `test_complete_invalid_challenge_format`). Fixed by replacing `if existing:` with `if existing is not None:` — this gives Pyright identical Optional narrowing without the semantics-breaking assertion
- **Dockerfile `common_utils` module missing from `COPY` block**: `library/common_utils/secret_resolver.py` was introduced in Phase 6a but the Dockerfile `COPY` block was not updated, causing the container to exit on startup with `ModuleNotFoundError: No module named 'common_utils'`. The API health check and all Docker-based tests failed until the `COPY library/common_utils/ /app/common_utils/` line was added
- **Dockerfile OpenSSL pin updated `3.5.5-1~deb13u2 → 3.5.6-1~deb13u1`**: Debian Trixie published `openssl 3.5.6-1~deb13u1` to `trixie-security`; `apt-get install -y openssl=3.5.5-1~deb13u2` fails with a downgrade error when the newer version is the only available candidate. Pin updated so `docker buildx build` succeeds on current Trixie mirrors
- **Playwright browser-binary fixture converts `Error` to `pytest.skip`**: `test_auth_ui_e2e.py`'s browser fixture raised an unhandled `playwright.Error: Executable doesn't exist` when Playwright's Chromium binary was absent (dev machine without `playwright install`), producing 6 fixture `ERROR` entries instead of clean `SKIP`s. Wrapped in `try/except` so missing-binary conditions produce `pytest.skip("playwright browser not installed")` — expected on dev machines; tests run fully on the test VM where browsers are installed
- **`ruff format` drift corrected in 3 files after test-pyright commits**: `library/common_utils/secret_resolver.py`, `library/tests/test_secret_resolver.py`, and `library/tests/test_audit.py` accumulated minor formatting drift during the iterative type-narrowing passes. `ruff format` normalised all three; no logic changes
- **`ruff format` applied to 9 pre-existing test files**: `library/tests/test_auth_admin.py`, `test_auth_backup.py`, `test_auth_database.py`, `test_auth_docker.py`, `test_auth_integration.py`, `test_auth_method_switch_claim_lifecycle.py`, `test_auth_performance.py`, `test_add_new_audiobooks.py`, and `test_batch_translate.py` had accumulated formatting drift prior to this release. Pure whitespace and quote normalization — no logic changes
- **`venv` synced to Dependabot-merged requirement floors and `mando` constraint corrected**: 3 production packages (requests 2.34.0→2.34.2, cbor2 6.0.1→6.1.1, idna 3.13→3.16) and 9 dev-tool packages were below `requirements.txt` / `requirements-dev.txt` floors after Dependabot PRs merged the floor bumps but the venv was not reinstalled. `mando` constraint in `requirements-dev.txt` also corrected from `>=0.8.2,<0.9` (which radon's `<0.8` transitive ceiling makes unresolvable) to `>=0.7.1,<0.8` — matching the existing inline comment and radon's cap. `pip-audit --strict` now reports "No known vulnerabilities found" (clears `GHSA-jjg7-2v4v-x38h` / idna < 3.15)
- **`upgrade.sh::create_backup` SC2012 suppressed on deliberate `ls` usage**: `shellcheck` flagged the `ls -1d ... | sort -r` filename-sort pattern at line 544 with SC2012 ("Use find instead of ls"). A `# shellcheck disable=SC2012` directive added inline — the existing comment block (lines 539–542) documents why lexical filename sort via `ls` is intentional (the `YYYYMMDD-HHMMSS` suffix makes filename order identical to mtime order without `find`'s non-deterministic directory-entry traversal)
...
- **CodeQL alerts [#535](https://github.com/TheBoscoClub/Audiobook-Manager/security/code-scanning/535) and [#536](https://github.com/TheBoscoClub/Audiobook-Manager/security/code-scanning/536) dismissed as false positives** (`py/reflective-xss` on internal validator helpers that return only static literal error strings). Alert #535: `library/backend/api_modular/translations.py` — `_validate_on_demand_request` (L801–820) returns only literal strings; `_parse_on_demand_ids` (L725–731) raises `ValueError` with static text; no user input reaches the response body. Alert #536: `library/backend/api_modular/utilities_crud.py` — `_validate_bulk_request` (L148–185) uses `entity_label` as a hardcoded literal from internal callers, not user input. Both dismissed via `gh api -X PATCH .../code-scanning/alerts/{535,536}` with detailed rationale comments. Verified state=dismissed
...
- **`ACKNOWLEDGEMENTS.md` license entries** for three runtime dependencies under the project's source-distribution model: `mutagen` (GPL-2.0+, audio metadata), `edge-tts` (LGPL-3.0+, Chinese TTS), `text-unidecode` (GPL/Artistic, CJK transliteration)

### Changed

- **Auth subsystem cyclic-import resolved — `auth_shared.py` extracted as leaf module**: `library/backend/api_modular/auth_shared.py` (756 lines) owns the shared contract every auth-family blueprint needs — the `auth_bp` Flask Blueprint, `_auth_db` handle, session-cookie helpers, all `@*_required` / `@*_if_enabled` decorators, `get_current_user`, `require_current_user`, and pure validation helpers. Nothing in `auth_shared.py` imports from any other `auth_*` module in `api_modular`, breaking the cycle that previously caused intermittent `ImportError` under certain test isolation orderings. `auth.py` re-exports every public name so existing `from .auth import X` callers and tests that patch `backend.api_modular.auth.X` continue to work unchanged. Commit `a182f5f4` (Wave A)
- **`auth.py` uses PEP 562 `__getattr__` for rebindable module-level state** — test isolation requires patching `auth._auth_db` and `auth._db_path` between test cases; a naïve re-export loses the binding when the test patches the name after import. The `__getattr__` hook defers the attribute lookup to `auth_shared` at call time rather than at import time, so patches to `auth_shared._auth_db` are visible through `auth._auth_db` in the same test. Commit `97c98c3a` (Wave B)
- **27 `innerHTML` assignments in `library/web-v2/js/library.js` and `library/web-v2/js/utilities.js` annotated with `// nosec:` comments** documenting their escape mechanism (e.g., `DOMPurify.sanitize()`, server-controlled strings, numeric values, `textContent` equivalents) — commit `3492cf6b` reduces false-positive CodeQL `js/xss` signals on reviewed and safe assignments
- **11 bare `# type: ignore` and `# noqa` suppressions carry rule codes + explanatory reasons** (`search_cjk`, `backfill_pinyin_sort`, `test_provider_agnostic`, `sampler-reconcile`) so future maintainers can distinguish genuine limitations from overlooked fixable issues — commit `f0190dfd`
- **45 ruff `I001` isort violations resolved**, `.dockerignore` augmented with `.snapshots/`, `.btrbk-snapshots/`, `library/.test-sandbox/` to exclude BTRFS and test-sandbox artifacts from Docker build context, `.semgrepignore` updated with `library/certs/*.key` — commit `2557bce2`
- **Dependency floor pins tightened**: `cbor2>=5.9.0 → >=6.0.0` (major; CBOR API stable), `gunicorn>=23.0.0 → >=26.0.0` (security/perf; was already running 26 in venv after the v8.3.10.5 in-venv bump), `eslint^10.2.1 → ^10.3.1` — commit `2557bce2`
- **`redirect_server.py` `httpd` variable latent unbound bug fixed** — `httpd` was declared in a `try:` block but referenced in the `except` and `finally:` branches without a `None` guard; pyright surfaced `reportPossiblyUnbound`; the variable is now initialised to `None` before the `try:` and the `finally:` guards with `if httpd:` — commit `2557bce2`
- **Stale "Vast.ai" comment in `scripts/audiobook-translations`** (`_probe_stt_providers` function header) replaced with provider-agnostic phrasing matching the v8.3.10.6 code reality — commit `a3056d19`
- **Type-narrowing cleanup — pre-existing pyright `reportOptionalSubscript` and dual-return-shape diagnostics resolved**: `library/backend/api_modular/translated_audio.py::_load_translated_audio_context` rewritten from a `tuple[Path, Path] | tuple[Response, int]` union return to a single `tuple[Path, Path]` happy-path return that raises a new `_TranslatedAudioContextError` for the four short-circuit cases (book-not-found, no subtitles, audio-already-exists). Pyright was previously unable to narrow the union at the call site, leading to "Object of type Response is not subscriptable" warnings on every downstream `.exists()` / Path operation. `library/backend/api_modular/streaming_translate.py::stream_chapter` now `assert`s the three `_parse_stream_request` outputs (`audiobook_id`, `chapter_index`, `locale`) are non-`None` after the error guard, restoring narrowing without `# type: ignore`. Tests updated: `library/tests/test_translated_audio.py::TestLoadTranslatedAudioContext` rewritten to use `pytest.raises(ta._TranslatedAudioContextError)` + `excinfo.value.status_code` for the 404 / no-subtitles / audio-already-exists assertions; the `_get_status()` Optional-subscript pattern (returns `dict | None`) gains explicit `assert status is not None` narrowing in 14 call sites across `TestJobStatusRegistry`, `TestAdminGenerateClosure`, and `TestUserRequestClosure`. `library/tests/test_data_migration_013_sampler_consolidation.py` consolidates three duplicate inline `_R` subprocess-result classes (which pyright flagged as conflicting redeclarations within the same function scope) into a single `_FakeProc` helper that takes an optional `stdout` constructor arg. All 4922 pytest cases still pass; ruff clean

### Removed

- **Vast.ai retired — RunPod is the sole STT/translation/TTS inference backend**: every Vast.ai code path, env var, and provider module has been expunged from the live codebase. Removed: `library/localization/stt/vastai_serverless.py` and `library/localization/tts/vastai_xtts.py` provider modules; `VASTAI_*` env vars (`AUDIOBOOKS_VASTAI_SERVERLESS_API_KEY`, `AUDIOBOOKS_VASTAI_SERVERLESS_STREAMING_ENDPOINT`, `AUDIOBOOKS_VASTAI_SERVERLESS_BACKLOG_ENDPOINT`, `AUDIOBOOKS_VASTAI_XTTS_HOST`, `AUDIOBOOKS_VASTAI_XTTS_PORT`) from `library/localization/config.py`; the `vastai` entry in `library/localization/gpu_health.py::_PROVIDERS` (the canonical helper that backs both `_probe_stt_warmth` in `streaming_translate.py` and `probe_gpu_instance_health` in `translation_monitor`); the dual-provider routing branches in `library/localization/pipeline.py::_remote_stt_candidates` and `_stt_by_explicit_name`; the Vast.ai serverless probe in `scripts/smoke_probe.sh::_probe_stt_providers`; the `AUDIOBOOKS_VASTAI_*` documentation stanzas in `install.sh` and `etc/audiobooks.conf.example`. Tests updated: `library/tests/test_provider_agnostic.py` collapsed from 4 provider scenarios to 2 (configured / not-configured) RunPod-only, `library/tests/test_gpu_health.py` simplified to RunPod-only aggregation, the entire `TestVastaiServerlessSTT` class removed from `test_localization_pipeline_coverage.py`, `test_stt_runtime_fallback.py` and `test_tts_factory.py` rewritten to exercise `WhisperSTT` / `XTTSProvider` instead of the deleted Vast.ai providers. Docs updated: `README.md`, `docs/ARCHITECTURE.md`, `docs/MULTI-LANGUAGE-SETUP.md`, `docs/STREAMING-TRANSLATION.md`, `docs/STREAMING-TRANSLATION.zh-Hans.md`, `docs/SAMPLER.md`, `docs/SERVERLESS-OPS.md`, `docs/TROUBLESHOOTING.md`, `docs/reference-system.yml`, `ACKNOWLEDGEMENTS.md`, `library/web-v2/about.html`, `library/locales/en.json`, `library/locales/zh-Hans.json`. Historical CHANGELOG entries that describe what was actually shipped at v8.3.x are intentionally preserved as release-history record. The maintainer scrapped Vast.ai for production some time ago; this release brings the codebase in line with that operational reality

### Fixed

- **CodeQL #531 — `proxy_server.py` CORS `Origin` header sanitized against CR/LF/control-character injection**: `_resolve_cors_origin()` in `library/web-v2/proxy_server.py` previously echoed the raw `Origin` request header into the `Access-Control-Allow-Origin` response header without validation. An attacker who could influence the `Origin` value (e.g., via a CRLF-injection vector) could inject arbitrary response headers. Fix: `_resolve_cors_origin()` now validates the `Origin` against a strict allowlist pattern (`^[a-zA-Z][a-zA-Z0-9+\-.]+(://)[^\r\n\x00-\x1f\x7f]*$`) and falls back to `*` on any value containing CR, LF, or other ASCII control characters — neutralising the injection surface entirely. 11 new tests in `library/tests/test_proxy_headers.py` cover the accepted/rejected boundary cases

- **iOS audio playback fix — sampler completion now consolidates `chapter.webm` so iOS WKWebView can play translated audio natively**: Qing's iPhone Chrome (Chrome iOS = WKWebView, same engine as Safari) hit a perpetual loading spinner on books like `A Dirty Job` that had only the 6-minute sampler translated. Root cause: iOS WebKit cannot play WebM containers via `MediaSource` (rejected by `MediaSource.isTypeSupported('video/webm; codecs="opus"')` even though the WebKit decoder supports the underlying Opus codec), so the per-segment MSE pipeline that drives non-iOS clients dead-ends on iOS. Books that already had `chapter_translations_audio` rows played fine via native `<audio src=…>` (e.g., `All The Light We Cannot See` — fully sampler-built); books without those rows had no fallback path. Fix: after `sampler_jobs.status` flips from `running` → `complete` in `library/backend/api_modular/streaming_translate.py::segment_complete`, the handler now calls `_consolidate_chapter_audio()` for every chapter that carries `origin='sampler' state='completed'` segments. The sampler scope can span chapters 0+1 for short opening chapters, so we consolidate every distinct `chapter_index` the sampler covered, not just chapter 0. Transition detection uses the running→complete UPDATE's `rowcount` so duplicate segment-complete callbacks (worker restart, retry) don't re-invoke ffmpeg on every call. The resulting `chapter_translations_audio` row carries `tts_provider='streaming'` and `duration_seconds` reflecting the sampler scope (~6 min, NOT the full chapter duration); the existing `INSERT OR REPLACE` semantic means later full-chapter consolidation (when the rest of the chapter streams in) transparently upgrades the row. Filtering of partial sampler rows in `library/backend/api_modular/translated_audio.py::get_book_translated_audio` stays in place — sampler rows are exposed to the player only after `sampler_jobs.status='complete'`. New data-migration `data-migrations/013_backfill_sampler_consolidation.sh` (`MIN_VERSION=8.3.10.6`) backfills `chapter_translations_audio` rows for books whose sampler completed BEFORE this fix shipped — the migration scans `sampler_jobs WHERE status='complete'`, iterates the distinct `streaming_segments.chapter_index` values for each `(audiobook_id, locale)`, and reuses the same `_consolidate_chapter_audio()` helper so the file layout, security checks, and ffmpeg invocation stay byte-identical to the live runtime (single source of truth, no duplication). New tests: `library/tests/test_streaming_tts_consolidation.py::TestSamplerCompletionTriggersConsolidation` (2 cases — happy-path + idempotency); `library/tests/test_data_migration_013_sampler_consolidation.py` (4 cases — single-chapter, multi-chapter, skip-existing, idempotent re-run)

- **Player handles "audio ended early" for partial sampler-only chapters — re-fetches with cachebust instead of advancing**: paired with the sampler-completion consolidation fix, the player needs to recognize when the audio it just finished is the partial sampler webm (~6 min) versus the true full chapter (~10–30 min). Without this, `audio.addEventListener("ended", …)` in `library/web-v2/js/shell.js` would fire at the partial duration and skip past the rest of the chapter into chapter `N+1`. Fix: new `_maybeRetryPartialChapter()` helper compares `audio.duration` against `(this.chapters[chapterIdx].end_ms - start_ms) / 1000` from `/api/audiobooks/<id>/chapters` (already loaded by `playBook`); if the played duration is below 95% of the expected chapter duration (5% tolerance for trailing-silence trim and rounding), the helper re-loads the same `/api/audiobooks/<id>/translated-audio/<chapter>/<locale>?_=<cachebust>` URL with a per-call cachebust query so HTTP caches refresh, then resumes from the position we just played to (via a `loadedmetadata` listener that restores `audio.currentTime`). Bounded retry: first retry is immediate; second retry waits 30 seconds (giving the streaming pipeline time to grow the consolidated file with newly-completed segments); after two retries with no growth, surface a one-time `shell.translationCatchingUp` toast and let the normal chapter-advance path proceed. The retry state is keyed on `${book_id}:${chapter_idx}` and cleared whenever the played duration meets the threshold. New i18n keys `shell.translationCatchingUp` added to `library/locales/en.json` and `library/locales/zh-Hans.json` (Chinese: `翻译还在生成中 — 请稍后刷新页面以继续收听。`). The new branch runs BEFORE the existing translatedEntries chapter-advance branch in the `ended` handler, so it covers partial chapters at any index (not just chapter 0)
...
- **Other minor/patch venv bumps (drift below latest)**: `certifi` 2026.2.25→2026.4.22, `click` 8.3.2→8.3.3, `filelock` 3.28.0→3.29.0, `greenlet` 3.4.0→3.5.0, `idna` 3.11→3.13, `librt` 0.9.0→0.10.0, `markdown-it-py` 4.0.0→4.2.0, `packaging` 26.1→26.2 (re-aligned to dev floor), `pathspec` 1.0.4→1.1.1, `pip` 26.1→26.1.1, `playwright` 1.58.0→1.59.0, `ruff` 0.15.11→0.15.12, `types-requests` 2026.04.08→2026.05.03, `urllib3` 2.6.3→2.7.0, `zope.event` 6.1→6.2. `mando` and `astroid` deliberately held back at 0.7.1 / 4.0.4 — radon and pylint upper-bound them respectively (Audiobook-Manager /test Phase 6)

### Fixed

- **Book-detail modal renders blank on Brave Android — body lock + modal sizing belt-and-suspenders fix (Audiobook-Manager-n9x)**: tapping a book tile on Browse All in Brave on Pixel 10 Pro XL portrait opened only the dim `rgba(0,0,0,0.8)` backdrop with no card body — no gold border, no close `×`, no content. Tap anywhere closed the modal because every click target was the modal element itself (no children intercepting clicks). Diagnosed by deploying a debug-instrumented build that painted `.modal-content` `getBoundingClientRect` + computed-style snapshot to a fixed-position banner: with `body { position: fixed; top: 0; overflow: hidden; width: 100% }` from the prior `_lockBodyScroll`, the modal-content was rendering at `top=2251` in a `701`-px tall viewport — 1,550 px below the visible area, then clipped by body's overflow:hidden. Brave Android (Chromium-derived) treats `position: fixed` on body as creating a containing block for descendants' `position: fixed`, contrary to CSS spec; the modal's `height: 100%` therefore resolved against body's full content box (~5,000 px) instead of the viewport, and `align-items: center` placed the centered card far below the screen. Pre-existing bug — also reproducible on prod (v8.3.10.4); Qing not affected because she lives in My Library on Chrome iOS, where the cached `myLibraryBooks` data path triggers a different working render path. **Fix**: two complementary changes. (1) `library/web-v2/js/library.js::_lockBodyScroll()` simplified to set only `html, body { overflow: hidden }` (no `position: fixed`, no save/restore scrollY, no body shift) — body now keeps its normal containing-block behavior so the modal's `position: fixed` resolves against the viewport as the spec intends. (2) `library/web-v2/css/modals.css::.modal { width: 100vw; height: 100vh; height: 100dvh }` (was `width/height: 100%`) — viewport units sidestep the containing-block question entirely so even if a future regression introduces a containing-block ancestor, modal sizing stays viewport-anchored. Diagnostic banner data captured before fix: `top=2251 height=459 left=20 vh=701 body.position=fixed body.top=0px body.overflow=hidden isCompact=true display=flex visibility=visible opacity≈0.3 transform=matrix(0.965,...,0.965)` (mid-`fadeScaleIn`-animation snapshot). Round-7 of the v8.3.10.5 staged release
- **`stream-translate-worker.py` claim ordering now prefers the player's active chapter within each priority tier**: pre-fix `claim_next_segment()` ordered by `(priority ASC, chapter_index ASC, segment_index ASC)`, so when the player advanced from chapter N to N+1 mid-book and both chapters had P0 pending rows, the worker kept claiming chapter N until it drained — leaving the user buffering on chapter N+1 for the duration of the older chapter's drain. Reproduced live on prod 2026-05-04 ~21:00 CDT (book 115056 / `All The Light We Cannot See`, zh-Hans, chapter 6 had 43 P0 pending segments while worker was still grinding 12 P0 pending segments in chapter 5; user saw `0/6` spinner with `active_chapter=6` for ~10 minutes despite worker producing one segment every ~10 sec). Fix: claim query's `ORDER BY` now inserts a tier between `priority ASC` and `chapter_index ASC` that prefers rows whose `chapter_index` matches the most-recent `streaming_sessions.active_chapter` for the same `(audiobook_id, locale)` (the row's existing `LEFT JOIN streaming_sessions sess` already brings `sess.active_chapter` into scope — no schema, API, or client change). The 3-tier P0/P1/P2 priority semantics from `handle_seek_impl` are preserved unchanged; the new tier only breaks ties WITHIN priority. Effectively makes the "active chapter wins" behavior that existing seek-driven reprioritization already produced indirectly the default for ANY chapter advance — including the v8.3.10 `translatedEntries`-end handoff path which goes through `POST /api/translate/stream` (NOT `/api/translate/seek`) and therefore never triggered the previous reprioritization. Regression tests in `library/tests/test_stream_translate_worker_priority.py` cover: active-chapter wins within tier, priority is still respected (P0 non-active still beats P1 active), back-compat when no session row exists, segment_index ASC ordering inside active chapter
- **Session grace period bumped 30 → 120 minutes to stop silent `PUT /api/position` 401s mid-listen**: `library/auth/models.py::Session.is_stale()` defaulted to `grace_minutes=30`, but audio streams (`/streaming-audio/*`, `/audio/*`) bypass `/api/*` and never refresh `last_seen` — a 30-minute listening session was guaranteed to expire the user's session before the next position-save, dropping their progress on the floor. New `Session.DEFAULT_GRACE_MINUTES = 120` constant is the single source of truth; `auth.py::get_current_user`, `maintenance_scheduler.py::run_auth_cleanup`, and `auth_cleanup.py::_cleanup_stale_sessions` all switched to the default (no explicit `grace_minutes=` kwargs). 5 new tests in `library/tests/test_session_grace_period.py` pin the value (119min not stale, 121min stale, persistent never stale, explicit override still works); existing `test_auth_cleanup_task.py` and `test_maintenance_scheduler_coverage.py` updated to assert call without `grace_minutes=` kwargs (Audiobook-Manager /test Phase 5d)
- **Session-expired 401 from `PUT /api/position` now surfaces a one-time toast instead of being silent**: `library/web-v2/js/shell.js::savePositionToAPI()` previously logged a `console.warn` on 401 and continued — the user had no idea their progress was no longer being saved. Now intercepts 401 specifically and routes through new helpers `_showSessionExpiredToast()` (5-minute suppression to avoid spam during a long stale-session run) and `_showShellToast()` (mirrors `library.js::showToast` pattern using a dedicated `#toast-container` div). Audio playback continues unimpeded — only progress persistence is affected, and the user gets a clear "Your session expired — sign in again to keep your progress saved." prompt. `library/web-v2/shell.html` now loads `css/notifications.css` and includes a shell-level `#toast-container` element (iframe content pages keep their own); `library/locales/en.json` + `library/locales/zh-Hans.json` add the new `shell.sessionExpired` translation key (Audiobook-Manager /test Phase 5d)
- **Proxy CORS headers no longer ship `Allow-Origin: *` without `Allow-Credentials: true`**: `library/web-v2/proxy_server.py::do_OPTIONS` and `_send_cover_response` emitted only `Access-Control-Allow-Origin`, but every web-UI fetch uses `credentials: "include"` to ride the session cookie — browsers require `Access-Control-Allow-Credentials: true` to expose the response, AND reject `Allow-Origin: *` alongside `Allow-Credentials: true` per the CORS spec. New `_resolve_cors_origin()` echoes the request `Origin` header instead of `*` when credentials are in scope (spec-compliant), `_send_cors_headers()` emits all three required headers together (`Allow-Origin`, `Allow-Credentials: true`, `Vary: Origin`), and a 7-test `TestCorsHeaders` class in `library/tests/test_proxy_headers.py` pins the contract (wildcard echoed, no wildcard with credentials, all three headers present). Existing `test_cors_preflight_response` still passes because no Origin header → wildcard preserved (non-browser caller path) (Audiobook-Manager /test Phase 5d)
- **`cryptography` 47.0.0 → 48.0.0 + `pyopenssl` 26.1.0 → 26.2.0**: `library/requirements.txt` bumped to address the Phase 5b "1 release behind latest" warning. `pyopenssl` had to bump alongside since 26.1.0 capped `cryptography<48`; 26.2.0 supports `cryptography>=46,<49`. `pip check` reports no broken requirements (Audiobook-Manager /test Phase 5b)
- **`zope.interface` venv-installed 8.3 upgraded to match the `>=8.4` floor pin** in `library/requirements-dev.txt`. No requirements file change — pin was already correct, drift was venv-only and resolved with `pip install --upgrade zope.interface` (Audiobook-Manager /test Phase 5b)
- **External `cloudflared-test`/`cloudflared-qa` systemd units fixed**: `StartLimitIntervalSec=300` and `StartLimitBurst=5` were placed in `[Service]` instead of `[Unit]`, so systemd silently ignored them and the rate-limiting was never applied. Both unit files at `/etc/systemd/system/cloudflared-{test,qa}.service` now have the keys in `[Unit]`. `sudo systemd-analyze verify` now exits 0 with no output (was emitting `Unknown key 'StartLimitIntervalSec' in section [Service], ignoring.`). Tunnels remained `Active: active (running)` throughout — `daemon-reload` was sufficient. NOTE: external host files only, not in the project repo (Audiobook-Manager /test Phase 5d)
- **ResourceWarnings on unclosed file/sqlite3 handles in tests reduced from 50+ to 38**: 7 file-serving tests now wrap responses in `try/finally: resp.close()` so Flask's `send_file` BufferedReaders release before pytest GC fires (`test_audio_path_resolution.py`, `test_audiobooks_extended.py`, `test_streaming_mse_route.py`, `test_subtitles_coverage.py`, `test_translated_audio.py`); `test_user_settings.py` now closes its `AuthDatabase` instances in fixture teardown; `test_proxy_server.py` defensively closes `HTTPError` mocks. Production code: `proxy_server.py::proxy_to_api` now closes the upstream `HTTPError`'s `fp` in a `finally` block (was leaking on every 4xx/5xx upstream); `_subprocess.py::run_with_progress` closes `Popen.stdout`/`stderr` PIPE streams in `finally`; `websocket.py::_poll_loop` wraps the `sqlite3.Connection` in `try/finally`. `conftest.py::flask_app` now runs `gc.collect()` at session teardown so module-level Flask blueprint state releases deterministically. Remaining 38 warnings are inherent to test fixtures that call `create_app()` themselves and re-initialize 12+ blueprints' module-level `_db_path` globals; cleaning those up requires a project-scope refactor of `init_*_routes` to register `teardown_appcontext` callbacks (out of scope for this audit) (Audiobook-Manager /test Phase 4a)
- **Translation-monitor GPU health check is no longer a stub — pessimistic by default**: `library/translation_monitor/probe.py::probe_gpu_instance_health()` previously returned `{"providers": {}, "any_healthy": True, "stub": True}` regardless of actual provider state, so the operator-facing `gpu=stub` log line carried no real signal and the 2026-05-04 incident saw a worker that was alive but starving the user's chapter while the monitor reported "ok". New canonical helper `library/localization/gpu_health.py::probe_all_streaming_providers()` performs an HTTP GET against each configured provider's `/v2/<endpoint>/health` (RunPod and Vast.ai serverless, both via existing `AUDIOBOOKS_RUNPOD_*` / `AUDIOBOOKS_VASTAI_SERVERLESS_*` env vars), with a 3-second timeout per provider and `urllib.error.URLError` / `TimeoutError` / `ValueError` swallowed to ready=0. The monitor's `probe_gpu_instance_health()` now calls the canonical helper, transforms the list-shape into the existing dict-keyed-by-name contract (`{providers: {runpod: {healthy, ready, endpoint}, ...}, any_healthy, stub: False}`), and writes a `gpu_probe_failed` event to `translation_monitor_events` on unexpected exceptions. Pessimistic semantics: `any_healthy` is False when no provider is configured or every probe returned 0 ready workers — the pre-fix optimistic True is the deeper bug that masked the 2026-05-04 incident. The same canonical helper is also wired into `library/backend/api_modular/streaming_translate.py::_probe_stt_warmth()` (replacing its inline `_probe_one` closure) so the API and the timer-driven monitor share one provider-discovery path — per the project's "Single Canonical Source — No Duplication" rule. The 60-second cache + `(cold, ready, providers)` tuple shape on the API side is preserved unchanged. 10 new unit tests in `library/tests/test_gpu_health.py` cover all four provider-config scenarios (none / RunPod-only / Vast.ai-only / both) plus pessimistic any_healthy semantics, with `urllib.request.urlopen` mocked so no real RunPod/Vast.ai traffic is generated; existing 17-test `library/tests/test_provider_agnostic.py` re-validates the API contract; 3 new tests in `test_translation_monitor.py` pin the no-stub behaviour. Caused the 2026-05-04 prod incident to go un-escalated for ~10 minutes while the monitor logged `gpu=stub` per pass with no other action
- **Translation monitor now emails the operator on chapter-starvation alerts**: pre-fix `alert_old_live_segments()` wrote one `live_age_alert` event per stale segment to `translation_monitor_events` and that was it — no human ever looked at the table during the 2026-05-04 incident, so chapter-6 segments backed up for 10+ minutes while the worker ground through chapter 5 with the alerts piling up unread. New `library/translation_monitor/notify.py::send_chapter_starvation_alert()` delivers a plain-text email per `(audiobook, monitor pass)` group via the existing SMTP env contract (`SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`/`SMTP_FROM`); recipient is `ADMIN_EMAIL` (the existing convention also used by `auth_email.py::admin_email`) with a `SMTP_FROM` fallback so a configured SMTP block alone is enough to receive alerts — no new env var introduced. The body lists the audiobook ID + title, locale, the player's current `active_chapter` from `streaming_sessions`, the chapters where stuck segments live, the count, and the oldest stale-segment age; when `active_chapter` is ahead of every stuck chapter the body explicitly flags "chapter-advance starvation" so the operator immediately knows it's the same class of bug the v8.3.10.5 worker fix addresses. Idempotent within a 60-minute cooldown via a new `live_age_alert_emailed` event type added to `translation_monitor.events.ALLOWED_EVENT_TYPES` — the cooldown is enforced via the audit table, not in-memory state, so it survives the timer-driven oneshot's process exit. SMTP failures (`smtplib.SMTPException`, `OSError`) are caught and logged; the monitor tick still exits 0 so a transient mail outage cannot crash the timer. `scripts/translation-monitor-live.py` calls into the new helper after `alert_old_live_segments()`, grouping returned segment IDs by `audiobook_id` so a single book with N stuck segments produces one email, not N. `etc/audiobooks.conf.example` updated to document `ADMIN_EMAIL` as the operator-alert recipient. 18 new tests in `library/tests/test_translation_monitor_notify.py` cover recipient resolution (3 cases), cooldown idempotency (4 cases), context gathering, body assembly, SMTP failure handling, and end-to-end success — all with `smtplib.SMTP` mocked so no real mail is sent
- **`library/locales/zh-Hans.json` curly-quote literals escaped to prevent JSON parse failure**: two keys added in Phase 5d (`help.sessionExpiredTitle` and `help.sessionExpiredDesc`) contained unescaped Unicode curly-quote characters (`"` / `"`) inside JSON string values — valid JSON but Python's `json.loads` (and the browser's `JSON.parse`) raised `SyntaxError` on the `"` character embedded mid-value because it was identical to the string-closing delimiter in some parser paths. Replaced with `\"` ASCII escaped double-quotes so both keys parse correctly in all conformant JSON parsers. Caught during pytest collection when `i18n.py` attempted to load the locale file and aborted on the malformed value (Audiobook-Manager /test Phase 4a round-3)
- **`library/translation_monitor` package switched from absolute to relative imports**: `__init__.py`, `notify.py`, and `probe.py` used `from translation_monitor.X import …` — absolute imports that require `library/` (the package's parent) to be on `sys.path`. Under the installed app's `sys.path` layout (maintenance scheduler, API worker) the parent IS on `sys.path`, but pytest's collection path adds `library/translation_monitor/` directly, making `translation_monitor` an unresolvable top-level name and blocking collection of all 31 test modules that import from the package. Converted all three files to relative imports (`from .X import …`) which resolve correctly regardless of which ancestor directory is the `sys.path` root. Caught during Phase 4a pytest collection sweep (Audiobook-Manager /test Phase 4a round-3)
- **`library/requirements-docker.txt` synced with `library/requirements.txt` for the round-1 `cryptography` 47→48 + `pyopenssl` 26.1→26.2 bumps**: round-1 raised the floors on the native side to address `CVE-2026-39892` (cryptography), `CVE-2026-27448`, and `CVE-2026-27459` (pyopenssl), but `requirements-docker.txt` was missed. Verified with a clean `docker buildx build --load` of `Dockerfile`: pre-fix the image installed `cryptography 47.0.0 + pyOpenSSL 26.1.0`, post-fix `cryptography 48.0.0 + pyOpenSSL 26.2.0`. Docker users were on the same CVE exposure as the pre-round-1 native install. Caught during Phase 9c Docker-validation cross-reference (Audiobook-Manager /test Phase 9c)
- **`docker-compose.yml` image tag synced from stale `8.3.10` to `8.3.10.5`**: the compose file's `image: ghcr.io/theboscoclub/audiobook-manager:8.3.10` line was last bumped at the v8.3.10 release and missed every patch since (8.3.10.1 → 8.3.10.5). Historical pattern (verified via `git log -p`) tracks every patch version exactly — the drift was a release-process slip, not an intentional minor-floor pin. Caught during Phase 9c Docker-validation cross-reference (Audiobook-Manager /test Phase 9c)
- **`Dockerfile` `ARG APP_VERSION` default synced from stale `8.3.10.4` to `8.3.10.5`**: CI builds pass `--build-arg APP_VERSION=$(cat VERSION)` so the published image LABEL is always correct, but the in-source default was still pointing at the previous patch. Local `docker build` without `--build-arg` would produce an image labelled with the wrong version. Caught during Phase 9c Docker-validation cross-reference (Audiobook-Manager /test Phase 9c)
...
- **`_audio_paths.py` defaults sourced from `library/config.py`**: rolled into the staged release. The original helper hardcoded `"/srv/audiobooks/Library"` / `"/srv/audiobooks/Supplements"` as fallback constants, which tripped `test_library_python_files_no_hardcoded_paths` and violated the project's no-hardcoded-paths rule (the canonical defaults already live in `library/config.py`). The helper now imports `AUDIOBOOKS_LIBRARY` / `AUDIOBOOKS_SUPPLEMENTS` from `config` so there is exactly one source of truth for the canonical defaults; the runtime env-var-at-request behaviour is preserved. Two regression tests in `test_audio_path_resolution.py` updated to assert the helper's `DEFAULT_*` constants equal `config.py`'s values rather than hardcoding `/srv/audiobooks` (the original hardcode would mis-fire whenever `config.py` resolves to a non-default root, e.g. dev mode). Discovered by /test --vm audit
- **`test_subtitle_user_request_resolves_foreign_db_path` re-inits subtitles `_db_path` against session DB**: in full-suite ordering, an earlier test fixture (auth lifecycle) re-binds `subtitles._db_path` to its own per-test tmpdir DB; that DB is gone by the time this test runs, so `_get_db()` raises `sqlite3.OperationalError: unable to open database file`. Test now calls `init_subtitles_routes(db_path, …)` against the session-scoped `flask_app`'s DB before issuing the request. Discovered by /test --vm audit
- **`test_api_mocked.py` stream tests mock `resolve_local_audio_path` helper**: the existing `test_stream_webm_format_remux` and `test_stream_webm_format_non_opus_ignored` pre-existing tests inserted DB rows with synthetic paths (`/test/path/webm_test.opus`) and patched `audiobooks.Path` to fool the on-disk existence check. The new helper lives in `_audio_paths.py` (its own module-private `Path`), so the patches no longer reach it and both tests began returning 404 instead of the asserted (200, 500). Each test now also patches `audiobooks.resolve_local_audio_path` to return the same mocked Path, restoring the original webm-remux exception-path coverage. Discovered by /test --vm audit
- **`docs/TROUBLESHOOTING.md` operator-hostname scrub**: removed the four operator-specific hostnames (`library.thebosco.club`, `qalib.thebosco.club`, `devlib.thebosco.club`, `testlib.thebosco.club`) from the `:80` redirect rationale section and replaced with a generic "deployments that front the app with a Cloudflare Tunnel" phrasing. `install.sh` UFW-reconcile comment likewise scrubbed (`devdocker.thebosco.club` → "Docker-stack hostname"). Per the agnostic-repo rule (no operator-specific hostnames in tracked files); discovered by /test --vm audit
- **`resolve_local_audio_path()` expands `${VAR}` references in env values**: the helper read `os.environ.get("AUDIOBOOKS_LIBRARY", default)` directly, but systemd's `EnvironmentFile=` directive does NOT perform shell expansion. With the default `audiobooks.conf` (`AUDIOBOOKS_LIBRARY="${AUDIOBOOKS_DATA}/Library"`), the gunicorn worker received the literal string `"${AUDIOBOOKS_DATA}/Library"`, the rebased candidate became `${AUDIOBOOKS_DATA}/Library/<rel>`, that path obviously didn't exist on disk, and `/api/stream/<id>` returned `{"error":"File not found on disk"}` — the very 404 the helper was meant to fix. Helper now runs the env value through `os.path.expandvars()` so the standard install convention resolves correctly. Caught only by the live VM rebase test (`/api/stream/<id>` against a synthetic foreign DB row through `testlib`'s Cloudflare path) — unit tests had used `monkeypatch.setenv("AUDIOBOOKS_LIBRARY", str(env_b_root))`, which seeded an already-expanded absolute path. New regression test `test_rebase_expands_unexpanded_shell_vars_in_env_value` reproduces the systemd literal-string scenario explicitly. Discovered by /test --vm audit Phase 10a
...
- **Play button silently broken on prod after v8.3.10.2 chapters-API wiring** (`Audiobook-Manager-8mm`, hot-patched via emergency revert): `library/web-v2/js/shell.js::ShellPlayer.playBook` issued `fetch('/api/audiobooks/<id>/chapters')` and called `_applyChapterButtonVisibility()` synchronously on the user-gesture stack BEFORE `await this.audio.play()`. The fetch's `.then()` microtask chain consumed the gesture activation, so `audio.play()` rejected with `NotAllowedError` on prod Chromium and the user saw "click play, nothing happens." Pre-existing comment in the same function (lines 545-547) explicitly warned about this exact pattern: "Cross-frame calls (iframe → parent) lose gesture activation if async operations (like API fetch) run first" — the v8.3.10.2 chapters wiring violated its own neighbour's contract. The 6ub chapter-skip-buttons feature is preserved by deferring both the chapters fetch and the visibility recompute into a `queueMicrotask` block that runs AFTER `audio.play()` resolves; chapter buttons light up one event-loop turn later than v8.3.10.2 attempted, but `audio.play()` keeps the gesture activation. Added `TestPlayBookGestureActivationOrder` (3 structural tests in `library/tests/test_chapter_nav_wiring.py`) so re-introducing async work between `audio.src` and `audio.play()` fails CI. Hot-patch trail: prod's `/opt/audiobooks/library/web-v2/js/shell.js` was reverted to the v8.3.10.1 sha `4df270fae9acedaf64cd60b63822a4a1262a5c5c` and cachebust bumped to `1777826705` across 13 web-v2 files — broken file preserved at `/opt/audiobooks/library/web-v2/js/shell.js.broken-8.3.10.2` for forensics. The new `/api/audiobooks/<id>/chapters` endpoint (server-side, issue 7bf) was unaffected by the regression and stayed live (Audiobook-Manager-8mm)
- **Audit-fix lint hygiene rolled into v8.3.10.3 staging**: `ruff format` reformatted 5 files surfaced by `/test --vm` (`library/tests/test_chapter_nav_wiring.py`, `library/tests/test_header_styling.py`, `library/tests/test_shell_js.py`, `.claude/diagnostics/browserstack_ios_repro.py`, `.claude/diagnostics/playwright_ios_repro.py`) — newly-added 8mm + bc0 regression tests landed without final formatter pass. `markdownlint` cleaned `AGENTS.md` (3× MD031 fenced-code blanks + 2× MD032 list blanks around `bash` blocks) and `CLAUDE.md` (MD012 trailing-blank lines). All 4989 tests still pass after format
...
- **Chapter-skip buttons now work on every playback path**: `library/web-v2/js/shell.js::ShellPlayer.playBook` fetches `/api/audiobooks/<id>/chapters` and reveals the `#sp-skip-back-chapter` / `#sp-skip-forward-chapter` controls whenever the response contains boundaries — covering the English single-stream `/stream/<id>` pathway that v8.3.10.1 had silently hidden the buttons on. The original v8.3.10.1 design only revealed the buttons for `streamingTranslate.isStreaming()` OR a populated `translatedEntries` array, leaving English audiobooks (the ~90% case) without any chapter-skip affordance — broken by-design rather than by-accident. New `_applyChapterButtonVisibility()` helper recomputes visibility after the async fetch resolves; `_skipBackChapter` and `_skipForwardChapter` consult `this.chapters` first (with the same 3-second double-tap-back UX from `Audiobook-Manager-9by`, now in milliseconds against `start_ms` / `end_ms`), falling through to the streaming MSE and `translatedEntries` paths so behaviour for those is unchanged. Reset of `this.chapters = []` at `playBook` entry prevents a previous book's boundaries from leaking into a new book before its fetch resolves. Stale-fetch guard re-checks `this.currentBook?.bookId === bookId` in both `.then` and `.catch` so a user switching books mid-fetch doesn't get the prior book's chapters applied. Source: Bosco's reaction post-v8.3.10.1 deploy ("It's insane to think that I did not intend for the FF/FB buttons and functionality to work regardless of locale") (Audiobook-Manager-6ub)
- **`upgrade.sh` Cloudflare cache purge no longer silently fails on every upgrade**: `purge_cloudflare_cache` was calling `audiobook-purge-cache` directly, but the script enforces `require_audiobooks_user` (added in `4bd9917d` for the audiobooks-owned config/DB security model) — every local and remote upgrade was emitting `error: audiobook-purge-cache must run as the audiobooks user. current: root (uid 0)` (remote) and `current: bosco (uid 1000)` (local) and the CDN was never purged. Fix: (a) `command -v audiobook-purge-cache` discovery now precedes the project-tree fallback so the operator-mode invocation finds `/opt/audiobooks/scripts/audiobook-purge-cache` (audiobooks-readable adjacent libs/configs) instead of the project copy (operator-owned 0600 `config.env` unreadable after privilege drop); (b) the invocation is wrapped in `sudo -u audiobooks env CF_GLOBAL_API_KEY=… CF_AUTH_EMAIL=… CF_ZONE_ID=… "$purge_script"` so the dropped-privilege child has the credentials it needs (audiobooks user has no `~/.config/api-keys.env` of its own); (c) early-exit when sudo to audiobooks isn't available, with `current_user == "audiobooks"` short-circuit so install.sh's edge case doesn't double-sudo. Verified end-to-end: actual Cloudflare API returned `Cache purged successfully` after the patch. Regression-guarded by 7 new tests in `library/tests/test_upgrade_purge_cache_user_gate.py`
- **`upgrade.sh` audit/cleanup no longer false-flags the `/usr/local/lib/audiobooks` compat symlink as a "legacy install directory"**: `install.sh` deliberately creates `/usr/local/lib/audiobooks → ${APP_DIR}/lib` as a backward-compat symlink for any scripts that still reference the old path, but the post-upgrade `audit_and_cleanup` step used `[[ -d "/usr/local/lib/audiobooks" ]]` (which follows symlinks) and warned `WARNING: Legacy install directory /usr/local/lib/audiobooks still exists` on every successful upgrade. Fixed by adding a `[[ -L ... ]]` test that precedes the `-d` test: a symlink resolving via `readlink -f` to either `${target}/lib` or `/opt/audiobooks/lib` is the expected compat shim and reports `Legacy compat symlink OK (→ …)`; only a real directory or a symlink pointing somewhere unexpected raises a warning
...
- **Chapter-level skip buttons in the player bar** (`Audiobook-Manager-9by`): two new buttons flanking the existing `±30s` controls, following the standard audiobook double-tap pattern (Apple Books, Audible, Pocket Casts):
  - `⏮` **Skip back to start of chapter** — tap mid-chapter restarts the current chapter (`audio.currentTime = 0`); tap within `RESTART_THRESHOLD_SEC` (3s) of chapter start jumps to the previous chapter instead
  - `⏭` **Skip forward to next chapter** — single tap jumps to the start of the next chapter
  - Buttons are `display:none` in `library/web-v2/shell.html` and revealed in `ShellPlayer::playBook` only when chapter boundaries are explicit (streaming MSE OR `translatedEntries` paths). The English single-stream path (`/stream/{id}`) has no client-side chapter info, so the buttons stay hidden — avoids exposing non-functional controls
  - Three integration paths handled: streaming MSE (routes through new `window.streamingTranslate.jumpToChapter(idx)` slow-path POST + `enterBuffering`), legacy `translatedEntries` (decrements/increments `translatedChapterIdx` and swaps `audio.src`), and end-of-`translatedEntries` skip-forward (hands off to `streamingTranslate.check(bookId, locale, nextChapter)` mirroring the v8.3.10.1 `ended`-handler logic so a book with sampler-tail + streaming-bulk doesn't dead-end)
  - New `streaming-translate.js` public API: `jumpToChapter(target)`, `getCurrentChapter()`, `getTotalChapters()`. `jumpToChapter` validates range (rejects `< 0`, rejects `>= totalChapters`), tears down the current MSE chain, clears preload state (stale `preloadedNextChapter` for `currentChapter+1` is wrong when user jumps to chapter 50), then POSTs `/translate/stream` with the target index and routes the response into `enterBuffering`
  - Refactor lock-in: `ShellPlayer::_loadTranslatedEntry(idx)` extracted from the previously-inline ended-handler chapter-advance code, now shared between `ended`-handler auto-advance, skip-forward, and skip-back-to-previous
  - i18n: new keys `player.skipBackChapterTitle` / `player.skipForwardChapterTitle` in both `en.json` and `zh-Hans.json` (curated translations per `feedback_in_app_docs_i18n_parity.md`, not DeepL fallback)
  - 17 new structural regression tests in `library/tests/test_chapter_nav_wiring.py` lock down the wiring (button ids, hidden-by-default, click handlers, public-API entry points, range validation, MSE teardown, preload clearing, i18n parity including a CJK-character-presence sanity check on zh-Hans values)
  - Surfaced during the v8.3.10 prod incident (2026-05-02): when Qing got stuck looping the last sampler chapter, there was no in-player way for her to jump past it. Chapter controls would have been the user-side recovery option — and are standard listener affordance regardless

### Fixed

- **`audiobook-translation-monitor-live.service` was being silently throttled by systemd StartLimit, dropping ~50% of its 30 s claim-reset/retry sweeps** (`Audiobook-Manager-oep`): the v8.3.10 GPU-pod monitoring tier ships two timer-driven services that watch for stuck/dead/failed claims — `audiobook-translation-monitor-live` (every 30 s) and `audiobook-translation-monitor-sampler` (every 5 min). The live tier had `StartLimitBurst=5 / StartLimitIntervalSec=300` — that's 1 start/min ceiling against a 2 starts/min timer, so the burst quota saturated at t≈150 s and the next 2-3 min of firings were silently blocked by systemd with `start-limit-hit`. Effect: the live-tier monitor was running ~5×/5 min instead of the intended 10×/5 min. `journalctl -u audiobook-translation-monitor-live` on prod 2026-05-02 showed continuous "Start request repeated too quickly" / "Failed with result start-limit-hit" stretches punctuated by single successful runs every ~5 min. Fix: bumped `StartLimitBurst` from 5 to 20 in `systemd/audiobook-translation-monitor-live.service`. 20 starts/300 s = 1 every 15 s, 4× headroom over the 30 s timer cadence; still enforces a runaway cap if the service entered a true fail-loop. `StartLimitIntervalSec=300` unchanged. Sampler tier untouched (its 5 min cadence in a 600 s limit window only fires 2 starts/window, well under `Burst=5`). Six new regression tests in `test_translation_monitor_systemd.py` parse both unit files and assert StartLimit budget ≥ timer cadence with margin, plus a runaway-cap upper bound and the global `IntervalSec≠0` guard from `feedback_systemd_startlimit.md`. Source: Bosco's question 'are the new services to watch for stuck, dead, or failed GPU pods working as expected in prod?' — answer was 'sampler yes, live no'

- **Book-detail modal landed outside the visible viewport on mobile when opened from a deep-scrolled main grid** (`Audiobook-Manager-v2z` — primary fix): Bosco reported on Brave Android (Pixel 10 XL Pro) that tapping a book cover from the main library grid opened the book-detail modal *below* the visible area — he had to scroll a screen-height to find it. Same modal opened correctly from "My Library" because that view typically opens at the top. The modal IS `position: fixed` in `modals.css` so it should viewport-overlay regardless of scroll, but Brave Android's dynamic bottom URL bar can resize the visual viewport mid-mount and the fixed modal lands in a position that's no longer visible. Fix: standard scroll-lock pattern — when the modal opens, freeze `body` at its current scroll position via `position: fixed; top: -<scrollY>px; overflow: hidden; width: 100%;` and restore on close. Two new static helpers `AudiobookLibraryV2._lockBodyScroll` / `_unlockBodyScrollIfLocked` handle the freeze/restore. Idempotent via a `data-modal-scroll-lock` body attribute (second lock call is a no-op so a modal-replace sequence doesn't double-save the lock position). `showBookDetail` calls lock when mounting the modal; both close paths (backdrop click + close button) call unlock; the existing-modal-cleanup at the top of `showBookDetail` also calls unlock so a new lock saves the user's real scrollY instead of the already-frozen 0. 7 structural regression tests in `test_modal_scroll_lock_wiring.py` lock the wiring (helpers exist, idempotency guard, ordering, restore behaviour, scrollTo:auto)

- **Mobile library view: redundant top pagination pushed first book card further below the fold** (`Audiobook-Manager-v2z` — partial fix): the `#top-pagination` block above the books grid duplicated the pagination already present at the page footer (`.pagination-section`). On mobile / small-tablet viewports this added ~53 px to an already-tall pre-grid stack (header + filters + results-info ≈ 730 px on 500 px-wide measurement), making users scroll further before reaching any book card. Hidden on `max-width: 768 px` via CSS-only rule in `library/web-v2/css/responsive.css`. Bottom pagination remains for navigation on all viewports. Note: this is the easily-confirmed slice of the broader v2z report; the underlying "centered with empty space before/after" symptom Bosco described couldn't be fully reproduced on the headless test browser (which enforces a 500 px minimum width, above the existing 480 px breakpoint that does the heavier compaction). v2z stays open as a follow-up pending a screenshot from his iPhone showing the specific dead-zone

- **Library Collections — fiction/non-fiction/subgenre/era classifications were mixed up** (`Audiobook-Manager-lpq`):
  - **Genre buckets**: `library/backend/api_modular/collections.py` had only 6 entries in `FICTION_GENRES` and 7 in `NONFICTION_GENRES` — the 13 canonical display names from `scanner/metadata_utils.py::GENRE_DISPLAY_NAMES`. The bulk-import path in `backend/import_to_db.py` preserves *raw Audible category names* verbatim (`"Literature & Fiction"`, `"Politics & Social Sciences"`, `"Religion & Spirituality"`, etc.), so most of the actual genre names in the prod DB weren't in either list. The `_classify_genre_children` else-branch defaulted everything to fiction, mixing 78 books of `"Politics & Social Sciences"` and 43 of `"Social Sciences"` into the Fiction bucket. Expanded both sets to cover the high-volume raw Audible names (Fiction: ~30 entries, Non-fiction: ~35 entries, no overlap). Added a `categorize_genre` keyword fallback for genres still in neither list — names matching non-fiction keywords (autobiography/biography/history/science/cooking/etc.) route to non-fiction; everything else stays fiction
  - **Eras backfill**: only 16 of 1,867 books on prod 2026-05-02 had any `audiobook_eras` row, despite `published_year` being populated for all 1,867. The bulk-import path inserts era rows from `book.get("eras", [])` only — empty for books where the importer didn't pre-compute eras. Scanner-direct path (`utils/db_helpers.py::insert_audiobook`) does compute eras via `determine_literary_era`, but most books came through bulk import. New data migration `data-migrations/011_backfill_audiobook_eras.sh` (MIN_VERSION 8.3.10.1) maps each book's `published_year` to the canonical era bucket using the same `<1800 / <1900 / <1950 / <2000 / <2010 / <2020 / 2020+` thresholds as `determine_literary_era`. Idempotent (skips books already with an era row). Auto-runs via `upgrade.sh::apply_data_migrations` and `install.sh` data-migrations loop on next deploy
  - 8 new regression tests in `TestGenreClassification` (high-volume Audible nonfiction/fiction set membership, `categorize_genre` fallback bucketing) plus the existing 5 generic tests. All 13 pass

- **Conversion Progress card showed 306% complete (5,829 in library / 1,901 sources)** (`Audiobook-Manager-94p`): `_count_opus_files` in `library/backend/api_modular/utilities_conversion.py` and `_collect_checksum_files` in `library/backend/api_modular/utilities_ops/hashing.py` both used `library_dir.rglob("*.opus")` recursively, which picked up every `.opus` file in the tree — including per-chapter translation artifacts under `Library/{Author}/{Book}/translated/{Book}.ch{NNN}.zh-Hans.opus`. Prod 2026-05-02 had 1,867 canonical book files at depth 7 + 3,962 chapter translation files at depth 8 = 5,829 total counted as "in library" against 1,901 source AAXC files = 306% "complete". Both functions now exclude any path with `translated` as a directory component (`f.parts` check, exact directory match — books with "Translated" in their title aren't accidentally excluded). Added `TestCountOpusFiles` (5 cases) + `TestCollectChecksumFilesExclusions` regression guards covering: canonical-only counting, 50-chapter `translated/` subdir exclusion, `.cover.opus` exclusion, the "Translated" book-title defensive case, and missing-directory zero-return. Source: Bosco screenshot 2026-05-02 — back-office Conversion Progress dashboard

- **`user_listening_history` rows ballooned because every 5-second position save created a fresh row** (`Audiobook-Manager-e62`): `library/backend/api_modular/position_sync.py::_update_listening_history` sets `session.ended_at = now` on every position save during active playback. The previous `ListeningHistoryRepository.get_open_session` predicate `WHERE ended_at IS NULL` then matched zero rows on the next save, triggering a fresh `INSERT INTO user_listening_history`. Result on prod 2026-05-02: `A Dirty Job` had 3,671 history rows, `The Long Fall` 1,102 rows — the back-office TOP LISTENED card surfaced these as listen counts (3671 ≈ ~5h continuous playback × 12 saves/min × 60 min). Widened the predicate to `ended_at IS NULL OR ended_at > now - 30 minutes` with a new `SESSION_IDLE_TIMEOUT_MINUTES = 30` class constant. Continuous activity within the idle window reuses the same row (UPDATE-only path); 30+ min idle creates a new session. SQL gotcha: stored `ended_at` values come from Python's `datetime.now()` (local time), while SQLite's `datetime('now')` is UTC, so the query uses `datetime('now', 'localtime', '-30 minutes')` to align — without `'localtime'` the comparison is off by the local-vs-UTC offset (~5h CDT) and the window never matches. New regression test `test_get_open_session_extends_within_idle_window` locks in the buggy-mutation simulation. Existing inflated rows on prod's auth.db are NOT auto-repaired by this commit (auth.db is SQLCipher-encrypted, needs separate tooling); the code fix stops the bleed and data repair will follow as a separate one-shot tool

- **`streaming_sessions.state` was perpetually stuck at `'buffering'` for every active session** despite the column existing in the schema and being filtered for `'streaming'` in WHERE clauses elsewhere — no code path in `library/backend/api_modular/streaming_translate.py` ever wrote `state = 'streaming'`. Surfaced during the v8.3.10 prod incident on 2026-05-02 when operators inspecting `streaming_sessions` saw `state='buffering'` for every book Qing was actually listening to (chapters 0–6 fully cached, MSE pipeline streaming, but the table claimed buffering). New `_mark_session_streaming(db, audiobook_id, locale)` helper performs the idempotent `'buffering' → 'streaming'` transition, called from two sites: (1) `_fully_cached_response` — when the active chapter is fully cached the player will start playing immediately, (2) the `'buffering'` response branch of `request_streaming_translation` when the existing bitmap already has `>= BUFFER_THRESHOLD` (6) completed segments — the client will transition to STREAMING state on receipt regardless. `handle_seek` already correctly flips back to `'buffering'` on uncached seeks, so no additional cleanup needed. Cosmetic but observable: makes the table reflect reality during ops debugging. New regression test `test_session_state_flips_buffering_to_streaming_when_cached` in `TestRequestStreamingTranslation` locks in the transition

- **Scanner ingested `translated/` chapter artifacts as standalone audiobooks**, surfacing per-chapter translation files (e.g. `Cat's Cradle.ch003.zh-Hans.opus`) as bogus library cards in the main grid (`Audiobook-Manager-2sw`): `library/scanner/add_new_audiobooks.py::_collect_audio_files` and `library/scanner/scan_audiobooks.py::_collect_files_by_format` both did `library_dir.rglob(f"*{ext}")` over the full library tree without excluding `translated/` — every scan re-ingested chapter artifacts as fresh `audiobooks` rows. Prod 2026-05-03 had 128 phantom rows out of 1,869 canonical (6.4% pollution), all under `Library/{Author}/{Book}/translated/`, with author/duration/cover-icon synthesized from the artifact filename. Mirrors the `Audiobook-Manager-94p` pattern but on the write side. Both functions now exclude any file whose `Path.parts` contains `'translated'` (exact directory-component match — defensive against books literally titled "Translated"). Added defense-in-depth `file_path NOT LIKE '%/translated/%'` to `library/backend/api_modular/audiobooks.py::_build_filter_clauses` (`GET /api/audiobooks`) and to all four queries in `library/backend/api_modular/grouped.py` (`GET /api/audiobooks/grouped` — author-main, author-orphan, narrator-main, narrator-orphan). New idempotent data migration `data-migrations/012_purge_translated_chapter_rows.sh` (`MIN_VERSION="8.3.10.1"`) `DELETE`s any existing affected `audiobooks` rows on operator installs in a transaction with `PRAGMA foreign_keys = ON` so all 15 child-table FK CASCADEs fire correctly. 8 new regression tests in `library/tests/test_scanner.py` (`TestCollectAudioFilesExclusions` + `TestScanCollectFilesByFormatExclusions`) cover translated-subdir exclusion, the 50-chapter case, and the "book titled Translated" defensive case. Prod was cleaned mid-incident via direct DELETE (128 rows purged, FK cascade dropped 128 orphan `audiobook_genres` rows; backup at `/var/lib/audiobooks/backups/audiobooks-pre-translated-purge-20260503-091048.db`). Source: Bosco screenshot 2026-05-03 — main library grid showing `Cat's Cradle.ch000.zh-Hans` through `.ch013.zh-Hans` as separate cards

- **Newly-ingested audiobooks left `sha256_hash` NULL because the post-insert hook lost its hash-generation call in the modular utilities_ops refactor** (`Audiobook-Manager-f5e`, regression of commit `a62281369` 2026-01-14): `library/scanner/add_new_audiobooks.py::_run_post_insert_hooks` fans out to enrichment / verification / translation hooks after a successful insert, but the hash-generation hook was never re-wired when `utilities_ops` was split into the modular package. Operators had to manually click "Generate Hashes" in the back office (`POST /api/utilities/generate-hashes-async`) for every new book, and books missing hashes were vulnerable to the source-protection rule in `CLAUDE.md` that requires partial-MD5 verification before deletion. Added `library/scripts/generate_hashes.py::generate_hash_for_book(audiobook_id, db_path)` — a single-book extraction of the bulk worker's per-book logic — and call it from the post-insert hook with try/except wrapping (failure logged to stderr "Hash generation error (non-fatal)" — never blocks a successful insert). 5 new regression tests in `library/tests/test_add_new_audiobooks.py` (`TestHashAutoGeneration` + `TestGenerateHashForBook`) cover: hook fires for new ingests, failure is non-fatal, helper returns hash for existing rows, returns None for missing rows, returns None when underlying file is missing. Prod backlog of NULL-hash books still requires the manual bulk-trigger one-shot — code fix prevents new NULL-hash rows from accumulating

- **`/api/user/new-books` returned no `Cache-Control` header, allowing browsers to apply heuristic freshness to per-user marquee state** (`Audiobook-Manager-5gf`): `library/backend/api_modular/user_state.py::get_new_books` and its `dismiss_new_books` companion (POST `/api/user/new-books/dismiss`) both returned bare `jsonify(...)` responses with no cache directives. `library/web-v2/js/marquee.js::initMarquee` does a single fetch on page load and never re-fetches; combined with browser heuristic caching of the no-Cache-Control JSON, after the prod chapter-artifact DB cleanup on 2026-05-03 the marquee continued to show phantom `.ch0NN.zh-Hans` titles in the user's open tab until they hard-refreshed. Both endpoints now set `Cache-Control: no-store, must-revalidate` and `Pragma: no-cache` (HTTP/1.0 fallback) on every response — per-user mutable state should never be cached anywhere. New `TestNewBooksNoCache` class in `library/tests/test_user_state_api.py` asserts both headers present on both routes. Audit of the wider `user_state.py` route surface (8 other per-user mutable endpoints — `/api/user/history`, `/api/user/downloads`, `/api/user/library`, `hide`/`unhide`, etc.) deliberately deferred to a separate release — a shared `@no_cache_response` decorator across all per-user routes is structural enough to deserve its own scope

- **Top Listened admin widget over-counted by ~700× and split books across stale audiobook_ids** (`Audiobook-Manager-ptj`, follow-through to the data-side of `Audiobook-Manager-e62`): `library/backend/api_modular/admin_activity.py::get_activity_stats` ran `SELECT audiobook_id, MAX(title), COUNT(*) FROM user_listening_history GROUP BY audiobook_id ORDER BY cnt DESC LIMIT 10`. Two compounding bugs: (1) Each `user_listening_history` row is a ~5-second position-update slice (98.3% of prod's 10,106 rows are 5-60 sec long) — `COUNT(*)` of slices is not "listens", it's position-update events. Prod 2026-05-03 showed "A Dirty Job: 3,671 listens" — actually 3,671 slices ≈ ~5 hours of real listening over March 1-24. (2) Library re-imports issue new `audiobook_id` values for the same book, but listening history retains the old IDs — "A Dirty Job" had two IDs in history (`114107` with 3,671 slices Mar 1-24, `115970` with 1 slice Apr 8). The `GROUP BY audiobook_id` treated them as different books. Fixed per Bosco's chosen semantic (Option C — total hours): replaced the query with `SELECT MAX(audiobook_id), title, SUM(COALESCE(duration_listened_ms, 0)) AS total_ms FROM user_listening_history WHERE title IS NOT NULL AND title != '' GROUP BY title ORDER BY total_ms DESC LIMIT 10` (collapses stale IDs by title; `MAX(audiobook_id)` picks the newest canonical id for the link). The `total_listens` aggregate metric is renamed `total_hours_listened` (float, hours) using the same slice-summing logic. Same `MAX(audiobook_id), title, GROUP BY title` collapse applied to the parallel `top_downloaded` query (downloads keep `COUNT(*)` since each row is one discrete event, not a slice). Front-end (`library/web-v2/js/utilities.js::loadActivityStats`): new `formatHours(ms)` helper renders milliseconds as `"Xh Ym"` / `"Xh"` / `"Xm"`; `renderTopList` refactored to accept a per-item formatter function. `library/web-v2/utilities.html` stat-card label changed from "Listens" to "Listening Time". 3 new regression tests in `library/tests/test_admin_activity_api.py::TestTopListenedSemantics` lock the slice-summing semantic, the stale-ID collapse, and the `total_hours_listened` float metric. No i18n parity needed — admin-only widget per `feedback_i18n_scope.md`. Source: Bosco screenshot 2026-05-03 — back-office Activity Audit dashboard showing implausible "3671 listens" for A Dirty Job

- **`scripts/bump-cachebust.sh` silently accepted `--help` (and any `-`-prefixed arg) as a literal cachebust stamp**, rewriting every `?v=…` reference in the 13 prod HTML files to `?v=--help` before being noticed. Root cause was the validation regex `^[a-zA-Z0-9._-]{1,32}$` — the `-` character is in the allowed set, so `--help` matched. Tightened to `^[a-zA-Z0-9._][a-zA-Z0-9._-]{0,31}$` (forbidding `-` as the first character) plus an explicit `--help`/`-h` short-circuit that prints usage and exits cleanly. Verified with 5 test cases: `--help` and `-h` show usage; `--foo` and `-bar` error and leave HTML files untouched; valid epoch stamps still work. Caught during the v8.3.10 prod incident on 2026-05-02 (operator running `bump-cachebust.sh --help` after a hot-patch deploy)
...
- **Conversion Progress card column labels truncated at right edge on mobile** (`Audiobook-Manager-xld`): `library/web-v2/css/utilities.css` `.conversion-stats` was a fixed `repeat(3, 1fr)` grid at all viewports, so on ≤480 px phones the rightmost cells of each row (`In Staging`, `Conversion Rate`) got squeezed below their label widths and clipped. Reflowed to `repeat(2, minmax(0, 1fr))` inside the existing `@media (max-width: 480px)` block — preserves all 6 metrics in a 2×3 grid, lets labels wrap (`white-space: normal; overflow-wrap: anywhere`), and tightens label/value sizing so density still reads at thumb distance

- **Conversion Progress showed "0% Calculating…" indefinitely when there were 0 source files** (`Audiobook-Manager-pgb`): `updateConversionProgressBar`, `getConversionRateText`, and `getETAText` in `library/web-v2/js/utilities.js` had no branch for the `source_count === 0` empty-library case — the percent calc divides by zero on the server side (clamped to 0), the rate tracker can never observe a delta, and the ETA fell through to the catch-all `"Calculating..."` string. Now the progress bar fills to 100 %, the percent shows `—`, the rate shows `—`, and the ETA reads `"No sources to convert"`. The container also gets the `conversion-complete` class so the bar styling matches a fully-converted library

- **`Index file not found: …` placeholder text in Find Duplicates was clipped on narrow viewports** (`Audiobook-Manager-889`): the long absolute path returned by the API (e.g. `/var/lib/audiobooks/checksums/library.idx`) overflowed `.duplicates-list .placeholder-text` horizontally on phones because the placeholder rule did not opt into wrapping. Added an inside-the-existing `@media (max-width: 480px)` rule in `utilities.css` that switches `.duplicates-list .placeholder-text` to `overflow-wrap: anywhere; word-break: break-word; text-align: left` so paths wrap inside the card

- **Service Status card said "3/3 RUNNING" while listing only a subset of audiobook services** (`Audiobook-Manager-b7p`): `library/backend/api_modular/utilities_system.py::get_services_status` enumerated only the 3-entry `SERVICES` allowlist (the *control* allowlist used by start/stop/restart endpoints), so the status badge ignored `audiobook-api`, `audiobook-proxy`, `audiobook-redirect`, `audiobook-stream-translate`, `audiobook-scheduler`, `audiobook-shutdown-saver`, both `audiobook-translation-monitor-*.timer` units, `audiobook-enrichment.timer`, and the `audiobook-upgrade-helper.path/.service` pair. Added a separate read-only `STATUS_SERVICES` roster (15 units total — covers `audiobook.target` + every `audiobook-*` `.service`/`.timer`/`.path` shipped under `systemd/`); `get_services_status` now reports the full roster while `_service_control` continues to gate write operations on the original `SERVICES` allowlist. Each entry carries a new `controllable` boolean, and `library/web-v2/js/utilities.js::loadServicesStatus` honours it — units excluded from the control allowlist render with a read-only `—` glyph instead of start/stop/restart buttons (so the UI doesn't expose actions that would 400 on the backend). `_get_service_status_entry` also now treats `waiting` (path units) as "active" and accepts `static`/`alias` from `is-enabled` so `.target` and `.path` units don't falsely report as inactive/disabled. Two new regression tests in `TestGetServicesStatus` lock in the full unit roster and the controllable-vs-readonly split

## [8.3.10] - 2026-04-28

### Added

- **Defensive auto-clear of stale `upgrade-status` lock when the helper crashes mid-upgrade** (`Audiobook-Manager-ic4`): if `audiobook-upgrade-helper.service` exits abnormally during an upgrade, `/var/lib/audiobooks/.control/upgrade-status` was left with `running:true` indefinitely, blocking every subsequent `/api/utilities/check-upgrade` call with HTTP 400 "An operation is already in progress" — the upgrade UI lock from this morning's prod incident (21-hour-old lock from a v8.3.8.13 crash blocked the v8.3.9 upgrade until manually truncated). Mirrors the structural pattern of `library/translation_monitor/probe.py::reset_stuck_live_claims`: same problem class (worker crashes mid-operation leaving a `running:true` flag), same fix shape (timeout + liveness gate, then idempotent reset). New `_clear_stale_lock_if_safe` and `_helper_service_active` helpers in `library/backend/api_modular/utilities_system.py` use a two-condition gate before clearing — status-file mtime older than `UPGRADE_HELPER_STALE_TIMEOUT_SEC` (default 1800s = 30 min, ≈3× safety margin over a normal 5–10 min upgrade) AND `systemctl is-active audiobook-upgrade-helper.service` returns inactive/failed. Fails closed: `systemctl` query timeouts/missing-binary treat the helper as active so a real lock is never falsely cleared. Auto-clear emits a `logger.warning` line so operators can see when it fired. Wired into `_check_not_running` so every API call that gates on the lock benefits — no separate timer required. 12 new tests in `library/tests/test_utilities_system.py::TestStaleLockAutoClear` and `::TestHelperServiceActive` cover all branches (stale + inactive → cleared, fresh + running → not cleared, stale + active → not cleared, not-running → no-op, end-to-end via `_check_not_running`, plus systemctl probe state classification including timeout/missing-binary fail-closed)

### Changed

- **Pinned Debian apt versions in `Dockerfile` for reproducible image builds** (`Audiobook-Manager-rdd`): six previously-unpinned packages now carry exact Trixie version strings — `ffmpeg=7:7.1.3-0+deb13u1`, `mediainfo=25.04-1`, `jq=1.7.1-6+deb13u1`, `curl=8.14.1-2+deb13u2`, `libsqlcipher-dev=4.6.1-2`, `openssl=3.5.5-1~deb13u2`. Captured 2026-04-27 from `apt-cache policy` inside the same `python:3.14-slim` base image used by `FROM` so the pins are guaranteed available at the same APT priority. Verified via `docker run --rm <base> apt-get install …` against the pinned versions — all six resolve and install cleanly. Removed the `# hadolint ignore=DL3008` directive on the `apt-get install` line (DL3008 is "Pin versions in apt get install" — now satisfied). Revisit the pins on the next Debian release (Trixie point release or jump to Forky); bumping any pin ALSO requires re-validating the CVE notes at the top of `Dockerfile`, since some pins (notably ffmpeg) track unpatched-upstream CVEs that the Trixie security team will eventually backfill

- **CI now runs ESLint against `library/web-v2/js/`** (`Audiobook-Manager-qnn`): wires the v8.3.9 flat config (`library/web-v2/eslint.config.js`) into the `ESLint (web-v2)` job in `.github/workflows/ci.yml`. The job runs `npm ci` from `library/web-v2/` and `npx eslint` against `js/` — fails the build on any error. Pre-existing warnings (113 of them, mostly `no-unused-vars` for catch-block error parameters) remain as informational; new errors block. Adds `library/web-v2/package.json` + `package-lock.json` for the lint toolchain (`@eslint/js@^10.0.1`, `eslint@^10.2.1`); `node_modules/` is already gitignored. The eslint config now declares `sourceType: "script"` (matches the classic `<script>` tag loading pattern, fixed 378 false-positive no-undef errors), exhaustive browser globals, project-internal cross-file globals (`api`, `library`, `formatLocal`, `formatRelativeTime`, etc.), and `no-redeclare: { builtinGlobals: false }` so the file that declares each cross-file export doesn't trip the rule. Real bugs found and fixed in the same pass:
  - **`library/web-v2/js/library.js::_trySessionRecover`** — `return true` followed by `await SessionPersistence.clear(); return false` left the cleanup unreachable. Stale tokens accumulated in localStorage after every failed restore. Cleanup moved into the `catch` block where it actually runs
  - **`library/web-v2/js/library.js`** — duplicate `escapeHtml(text)` class member at line ~2468 silently overrode the canonical implementation (which uses `text == null` to preserve `"0"` and falsy-but-meaningful inputs); the duplicate's `if (!text)` form would have mis-handled `text === 0` if such input ever reached it. Duplicate removed
  - **`library/web-v2/js/library.js::createPaginationHTML`** — `let pages = []` was immediately overwritten in every branch; changed to `let pages` so the throwaway initializer doesn't lurk
  - **`library/web-v2/js/account.js::openAccountModal`** — the wrapper at the bottom of the file (`var _origOpenModal = openAccountModal; openAccountModal = function () { _origOpenModal(); loadPreferencesIntoModal(); };`) was DEAD CODE: `addEventListener("click", openAccountModal)` at line ~328 had already captured the original function reference, so the wrapper never fired and saved browsing preferences were never auto-loaded. The `loadPreferencesIntoModal()` call moved inline into the original function (after the auth check, where `authenticated` is implicit); dead wrapper deleted
  - **`library/web-v2/js/version-poller.js::shouldPromptFor`** — the dismiss-backoff branch referenced `dismissedFor` / `dismissedAt` / `BACKOFF_AFTER_DISMISS_MS` that were never declared in the module, so `dismissedFor === newVersion` always compared against `undefined` and the early-return never fired. The whole branch was vestigial — the file's docstring explicitly says (2026-04-25 decision) "every acknowledgement reloads," removing the stay-on-old-version escape hatch. Branch deleted to match the design
  - **`library/web-v2/js/utils.js`** — added the missing `formatRelativeTime(d)` function (locale-aware "X minutes ago" / "Y hours ago" via `Intl.RelativeTimeFormat` with absolute-time fallback). It was called from `utilities.js:2927` but never defined, throwing `ReferenceError` whenever a recent activity row needed to render its relative timestamp

- **`vm-test-manifest.json` opts in to `assert_enabled_timers_active` regression guard** (`Audiobook-Manager-cf2`): pairs with the `enable --now` fix in `Audiobook-Manager-hp2`. After `install_on_pristine_vm` in `/test` Phase 10b runs the project's install command on the VM, it now queries systemctl for every `/etc/systemd/system/audiobook-*.timer` and asserts each enabled timer is also `active` — catching the "enable without `--now`" upgrade-bug class going forward. Shutdown-only units (`WantedBy=halt.target reboot.target shutdown.target` *only*) are auto-exempted by the phase logic (those are designed to activate at shutdown, not at boot, so 'enabled but inactive' is the correct steady state for them). Skill change lives in `claude-test-skill` repo (`skills/test-phases/phase-10b-vm-lifecycle.md`); this project just declares the opt-in via the new `vm_testing.install.assert_enabled_timers_active.unit_pattern` field

### Fixed

- **`upgrade.sh`: api start-limit-hit race during the new `enable --now` phase eliminated by reordering** (`Audiobook-Manager-0q0`): when the v8.3.10 hp2 fix changed `enable_new_services` to use `enable --now` instead of bare `enable`, services started eagerly DURING the upgrade rather than waiting for the late `start_services` phase. This exposed a pre-existing race: `verify_installation_permissions` (which `chown -R audiobooks` and `chmod 755` on the entire install tree) ran AFTER `enable_new_services` in `do_upgrade`, so eager service starts saw a transient permission state (root-owned files left over from `apply_config_migrations`) and `audiobook-api` failed CHDIR into `/opt/audiobooks/library/backend` until it hit `StartLimitBurst` and stayed in failed state for ~1 minute. Discovered on QA's v8.3.9 → v8.3.10 native upgrade — smoke probe correctly reported "API health unreachable" and the operator had to `reset-failed && start` manually. Two-part fix: (1) moved `verify_installation_permissions` to run BEFORE `enable_new_services` so perms are correct at the moment `enable --now` triggers each start; (2) added a defensive `reset-failed`-and-retry-once loop inside `_enable_unit_smart` (mirrored in both `upgrade.sh` and `install.sh`) so any future transient issue self-recovers without operator intervention. Verified by re-running the upgrade against QA — clean start, no CHDIR errors, no StartLimit hits

- **Chinese auto-advance at chapter end on iPhone (and any iOS Chrome/Safari device)** (`Audiobook-Manager-dwa`): when a streaming-translated chapter ended, playback stopped and the listener had to manually tap play to resume. Root cause was iOS's user-gesture-chain enforcement: the post-`ended` chain in `library/web-v2/js/streaming-translate.js::advanceChapter` (POST `/translate/stream` → bitmap fetch → MSE rebuild → `sourceopen` → `audio.play`) crossed too many async ticks for iOS to consider the new playback as continuation of the user's original Play tap. The play() rejection was silently swallowed by `.catch(function () {})`. English (cached) auto-advance worked because `shell.js`'s ended handler does a single synchronous `src` swap. Fix: a new `maybePreloadNextChapter()` runs on every `timeupdate` event and, when fewer than `PRELOAD_TRIGGER_SEC` (10s) remain in the current chapter, eagerly POSTs `/translate/stream` for chapter+1 in the background. The response is stashed in `preloadedNextChapter`. At chapter end, `advanceChapter` checks for the preloaded bitmap and takes a fast path — straight to `enterBuffering` without any network round-trip — so `audio.play()` runs in the same task as the `ended` event and iOS allows the continuation. Slow path (preload missed because chapter shorter than the trigger, or fetch failed) falls through to the original POST flow with no regression. Cleanup paths (`enterIdle`, post-consume in `advanceChapter`) detach the timeupdate listener and clear `preloadedNextChapter` so a book-switch never splices cross-session data into the new playback

- **Subtitle side-panel: brighter active-cue highlight + faster scroll resync** (`Audiobook-Manager-ic5`): the active transcript line was hard to find at a glance. Previous `.transcript-cue.active` styling (`library/web-v2/css/i18n.css:403, 568`) used `background: rgba(197, 165, 90, 0.15)` — only 15% gold tint over the Art Deco dark wood — which combined with `parchment` (`#eddcc4`) text colour read as faintly-shaded rather than visibly playing. Bumped to 35% gold fill, brighter foreground (`#fff5d8` ≈ off-white), and a 12px box-shadow gold glow so the line is unmistakable on iPhone in bright light. Same change applied to the bilingual-panel variant for consistency. Separately, the side-panel auto-scroll was falling 4 full seconds behind narration after any user nudge — `USER_SCROLL_PAUSE_MS` reduced from 4000 ms to 1500 ms, and `scrollIntoView` switched from `behavior: "smooth"` to `behavior: "auto"` (smooth-scroll animation was sluggish enough on iPhone that the panel was perpetually trailing the narration; auto snaps in <16 ms and stays in lockstep)

- **`audiobook-shutdown-saver.service` now actually saves tmpfs staging on shutdown** (`Audiobook-Manager-c8l`): the unit declared `User=root` but its `ExecStart=/usr/local/bin/audiobook-save-staging` calls `require_audiobooks_user()` which exits 1 unless the invoking UID matches the `audiobooks` user. Result: every shutdown attempt FAILED with `error: audiobook-save-staging must run as the audiobooks user` and tmpfs files were never persisted to the library — verified via journalctl during v8.3.9 prod upgrade verification, which surfaced the same `Failed to start audiobook-shutdown-saver` from the upgrade-helper's restart loop. Fix: switch to `User=audiobooks Group=audiobooks` so newly-moved files inherit correct `audiobooks:audiobooks` ownership and the script's user-gate passes. The `systemctl stop audiobook-converter/mover` calls in the script (`scripts/audiobook-save-staging:69-71`) now silently no-op when invoked without privilege — acceptable because the shutdown sequence stops those services shortly afterwards anyway. Regression guard: `library/tests/test_shutdown_saver_user.py` (3 tests) asserts `User=audiobooks`, `Group=audiobooks`, and the canonical `ExecStart` path so the unit can't drift back
...
- **VM map pristine path drift**: `~/.claude/config/project-vm-map.json` referenced the retired `test-audiobook-cachyos.pristine-2026-04-03.qcow2` in two places; corrected to the current `pristine-2026-04-17.qcow2` so `/test` Phase V can actually find the backup it restores from
- **Stale v8.3.8 release-staging tree removed**: gitignored but still on disk at `build/audiobook-manager-8.3.8/` (13 MB) plus `build/audiobook-manager-8.3.8.tar.gz` and its `.sha256` — orphans from the previous release that never got cleaned up; `build/` removed entirely
- **Pylint annotations on translation_monitor**: narrowed the bare `except Exception` in `library/translation_monitor/db.py::resolve_db_path` to `except (ImportError, AttributeError)` so the config-import fallback no longer swallows real bugs; added `# pylint: disable=too-many-arguments` with a one-line rationale to `library/translation_monitor/events.py::log_event` (the 8 named columns are the audit-trail signature, not a refactoring opportunity); added file-level `# pylint: disable=invalid-name,broad-exception-caught` plus `main()` docstrings to `scripts/translation-monitor-live.py` and `scripts/translation-monitor-sampler.py` (hyphenated module names are intentional; the broad-except is the documented "don't crash the timer" pattern)
- **Bandit B608 false-positive suppression**: `library/tests/test_translation_monitor.py:93,120` and `library/translation_monitor/probe.py:262` carry static-SQL-with-`?`-placeholders patterns that trip B608's regex; suppressed with explicit `# nosec B608 - …` rationale on the line where bandit registers the AST node (the f-string fragment, not the surrounding `conn.execute(`). Required hoisting the SQL into a named `sql_insert_seg` / `sql_insert_job` variable in the tests so the nosec lands on bandit's exact registration line — `conn.execute( # nosec` does not work because bandit anchors the node mid-call. `scripts/sampler-reconcile.py:255` gained a `# nosemgrep: dangerous-os-exec-tainted-env-args` next to its existing `# nosec B606`
- **Cryptography pin bumped to 47.0.0**: `library/requirements.txt` and `library/requirements-docker.txt` raised from `cryptography>=46.0.7` to `>=47.0.0`. The cascade required bumping `pyopenssl>=26.1.0` (26.0.0 carried `cryptography<47`); `pip check` confirms the dependency tree is consistent
- **Markdownlint config updated**: `.markdownlint.json` `MD013` was set to an 800-char line-length that triggered 20+ errors on existing CHANGELOG bullets (the new release entries routinely cross 1000 chars by design); switched to `MD013: false` and added `MD041: false` so a badge-first README doesn't trip first-line-heading. `MD024`/`MD033`/`MD036`/`MD060` disables preserved
- **ESLint flat config seeded for `library/web-v2/js/`**: new `library/web-v2/eslint.config.js` (ES-modules, `@eslint/js` recommended, browser globals, `no-unused-vars: warn`, `no-console: off`). Wiring into a runnable lint pass is filed as a follow-up bd issue — npx-launched ESLint 10 walks parent-ward into a malformed package.json which needs its own remediation
- **Translation-monitor sys.path bootstrap (Phase 10a P0 release blocker)**: `scripts/translation-monitor-live.py` and `scripts/translation-monitor-sampler.py` were inserting `_HERE.parent` (e.g. `/opt/audiobooks`) into `sys.path` instead of `_HERE.parent / "library"` where the package actually lives. Result: every systemd timer fire crashed silently with `ModuleNotFoundError: No module named 'translation_monitor'`, leaving the entire two-tier monitor non-operational on a fresh v8.3.9 install. The fix swaps the candidate iteration order so the package directory (`<root>/library`) is checked first, and inserts that exact path. The unit-test pair (`test_live_script_imports_cleanly` / `test_sampler_script_imports_cleanly`) didn't catch this because in-process `importlib.spec_from_file_location` inherits the test runner's `sys.path` — added `test_live_script_runs_in_clean_env` and `test_sampler_script_runs_in_clean_env` as subprocess-based regression guards that exec the scripts with `PYTHONPATH` stripped, mirroring `systemd ExecStart=`
- **Backfill `chapter_translations_audio` for orphan `chapter.webm` files**: a window in earlier 8.3.x releases produced consolidated `chapter.webm` files at `<streaming_audio_root>/<book>/ch<NNN>/<locale>/chapter.webm` without inserting the corresponding `chapter_translations_audio` row. The `GET /api/audiobooks/<book>/translated-audio?locale=xx` endpoint then omitted those chapters, so the front-end legacy player loaded the FIRST present chapter index — which on books missing ch=0 meant skipping the intro entirely and starting at ch=1 from `position=0`, matching the symptoms of the prod incident on 2026-04-27. New data migration `data-migrations/010_backfill_chapter_translations_audio.sh` (`MIN_VERSION="8.3.9"`) walks the streaming-audio tree and `INSERT OR IGNORE`s a row for every `chapter.webm` whose `(book_id, chapter_index, locale)` tuple is missing, with `ffprobe`-derived `duration_seconds`. Idempotent — repeated runs are no-ops
